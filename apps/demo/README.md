...
| 层级 | 内容 | 位置 |
|---|---|---|
| **Layer 1** — 组件库 | shadcn/ui 组件（Button, Dialog, Input, Select, Tabs 等） | [`src/components/ui/`](src/components/ui/) |
...
1. 复制 `src/components/ui/` 到你的项目
2. 复制 `src/index.css` 中的 tokens 和 classes 到你的全局样式
3. 安装依赖：`npm install class-variance-authority clsx tailwind-merge`
4. 复制 `src/lib/utils.ts`（提供 `cn()` 工具函数）

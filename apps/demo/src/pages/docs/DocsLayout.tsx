...
function SidebarLink({ item, currentPath, onNavigate }: { item: SidebarItem; currentPath: string; onNavigate?: () => void }) {
...
function SidebarSection({ group, currentPath, onNavigate }: { group: SidebarGroup; currentPath: string; onNavigate?: () => void }) {
  return (
    ...
    <>
      ...
      <nav className="flex-1 overflow-y-auto px-4 pb-4">
        {SIDEBAR_GROUPS.map(group => (
          <SidebarSection key={group.label} group={group} currentPath={currentPath} onNavigate={onNavigate} />
        ))}

        <div className="!mt-4 !mb-2 border-t border-[#e5e5e8]" />

        <a href="/openclaw" className="w-full flex items-center justify-between px-2.5 py-1.5 rounded-md hover:text-[#1f2328] hover:bg-[#ececef] transition-colors text-[13px] text-[#636366]">
          <span>Landing</span>
      ...
      {crumbs.map((crumb, i) => {
        ...
        return (
          <span key={crumb.path + i} className="flex items-center gap-1">
            {i > 0 && <BreadcrumbSep size={10} className="text-text-placeholder" />}
            {isLast ? (
              <span className="text-text-secondary font-medium">{crumb.label}</span>
            ) : (
              <Link to={crumb.path} className="hover:text-text-primary transition-colors">{crumb.label}</Link>
            )}
          </span>
        );
      })}
  ...
  return (
    <nav className="mt-12 pt-8 border-t border-border">
      <div className="grid grid-cols-2 gap-3 max-w-2xl">
        {prev ? (
          <Link to={prev.path} className="flex flex-col items-start p-4 rounded-xl border border-border hover:border-border-hover hover:shadow-sm transition-all text-left group">
        ...
        {next ? (
          <Link to={next.path} className="flex flex-col items-end p-4 rounded-xl border border-border hover:border-border-hover hover:shadow-sm transition-all text-right group">
            <span className="text-[13px] font-semibold text-text-primary group-hover:text-accent transition-colors flex items-center gap-1">
              {next.title}
      ...
      <nav className="sticky top-0 z-50 border-b backdrop-blur-md border-border bg-surface-0/90">
        <div className="flex justify-between items-center px-4 sm:px-8 mx-auto max-w-5xl h-14">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-[11px] text-text-muted bg-surface-2 px-1.5 py-0.5 rounded font-mono">v0.1</span>
          </div>
        </div>
      </nav>
        ...
        <>
          ...
          <aside className="fixed inset-y-0 left-0 z-50 w-72 bg-[#f5f5f7] border-r border-[#e5e5e8] flex flex-col lg:hidden overflow-y-auto">
            <div className="flex items-center justify-between px-5 pt-4 pb-2">
              ...
              <button type="button" onClick={() => setMobileOpen(false)} className="p-1 rounded text-text-muted hover:text-text-primary">
                <X size={16} />
              </button>
            </div>
          ...
          <aside className="hidden lg:block">
            <div className="lg:sticky lg:top-20 lg:self-start space-y-5 pt-8">
              ...
              <div className="rounded-2xl border border-border bg-surface-1 px-4 py-4 flex flex-col gap-2">
                <div className="text-xs font-semibold text-text-muted uppercase tracking-wide">Feedback</div>
                {feedback === 'submitted' ? (
                  <div className="text-[13px] text-text-secondary mt-1">Thanks for your feedback!</div>
                ) : (
                  <>
                    ...
                    {feedback === 'yes' && (
                      <div className="mt-1 text-[11px] text-text-tertiary">Thanks! Glad it helped.</div>
                    )}
                    {feedback === 'none' && (
                      <div className="mt-1 text-[11px] text-text-tertiary">Your answer helps us improve these docs.</div>
                    )}
                  </>
                )}
              </div>

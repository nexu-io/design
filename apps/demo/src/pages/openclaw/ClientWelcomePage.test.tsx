...
describe('ClientWelcomePage', () => {
  ...
  it('separates product positioning on the left from entry methods on the right', () => {
    ...
    expect(screen.getByText('nexu 让 OpenClaw 变成一个安装即可使用的完整产品。飞书文档、日历、审批等工具能力开箱可用，同时接入 Claude、GPT 等顶级模型，所有工作都在一个工作台里完成。')).toBeTruthy();
    ...
    expect(screen.queryByText('选择一种方式开始使用 nexu。你可以使用官方账号获得完整体验，或直接连接你自己的 API Key。')).toBeNull();
  ...
  it('opens workspace settings with anthropic selected from the secondary entry', () => {
    ...
    expect(navigateMock).toHaveBeenCalledWith('/openclaw/workspace?view=settings&tab=providers&provider=anthropic');
  });

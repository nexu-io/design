...
export default function AuthPage() {
  ...
  const handleGitHubAuth = () => {
    setLoading('github');
    setTimeout(() => { setLoading(null); onAuthComplete(); }, 800);
  };

  const handleGoogleAuth = () => {
    setLoading('google');
    setTimeout(() => { setLoading(null); onAuthComplete(); }, 1200);
  };

  const handleEmailSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setEmailError('');
    if (!email.trim()) { setEmailError(t('auth.error.email')); return; }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { setEmailError(t('auth.error.emailInvalid')); return; }
    if (!password.trim() || password.length < 6) { setEmailError(t('auth.error.password')); return; }
    setLoading('email');
    setTimeout(() => { setLoading(null); onAuthComplete(); }, 1200);
  };
              ...
              <div className="bg-surface-1 border border-border rounded-2xl p-7 sm:p-8 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
                ...
                <div className="space-y-2.5">
                  ...
                  <div className="flex items-center gap-3 py-1">
                    ...
                    <span className="text-[11px] text-text-muted uppercase tracking-wider select-none">{t('auth.or')}</span>
                    <div className="flex-1 border-t border-border" />
                  </div>
                    ...
                    <div>
                      <form onSubmit={handleEmailSubmit} className="space-y-2.5">
                        ...
                        <input
                          ...
                          placeholder={mode === 'register' ? t('auth.password.placeholder.register') : t('auth.password.placeholder.login')}
                          className="w-full h-11 px-3.5 rounded-lg border border-border bg-surface-0 text-[14px] text-text-primary placeholder:text-text-muted focus:outline-none focus:border-text-tertiary focus:ring-1 focus:ring-text-muted/20 transition-colors"
                        />
                        {emailError && (

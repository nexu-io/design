...
export function ChatMsg({ from, name, children }: {
  ...
  return (
    <div className='flex gap-2 mb-2.5'>
      ...
      <div className='flex-1 min-w-0'>
        {name && from === 'other' && <div className='text-[11px] text-text-tertiary mb-0.5'>{name}</div>}
...
export function ChatWindow({ title, badge, children }: {
  title: string
  badge?: string
  children: ReactNode
}) {
  return (
    <div className='bg-surface-1 border border-border rounded-xl overflow-hidden shadow-sm'>
      <div className='flex items-center gap-2 px-4 py-2.5 border-b border-border bg-surface-0'>
        <div className='w-5 h-5 rounded-full bg-clone/20 flex items-center justify-center text-[10px]'>😊</div>
        <span className='text-xs font-medium text-text-primary'>{title}</span>
        {badge && <span className='text-[10px] text-text-muted bg-surface-3 px-1.5 py-0.5 rounded'>{badge}</span>}
...
export function ScenarioSection({ tag, tagIcon: Icon, title, desc, features, chatContent, reverse }: {
  tag: string
  tagIcon: LucideIcon
  title: string
  desc: string
...
export function SectionHeading({ tag, title, desc }: {
  tag?: string
  title: string
  desc?: string
}) {
  return (
    <div className='text-center mb-14'>
      {tag && <div className='text-[11px] font-semibold text-clone mb-3 tracking-widest uppercase'>{tag}</div>}
      <h2 className='text-2xl font-bold text-text-primary tracking-tight'>{title}</h2>
      {desc && <p className='text-sm text-text-tertiary mt-3 max-w-lg mx-auto leading-relaxed'>{desc}</p>}
    </div>
  )
}

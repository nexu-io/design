  ...
  return (
    <div className="w-full h-full flex items-center justify-center relative overflow-hidden">
      ...
      <div className="relative text-center max-w-3xl px-8">
        ...
        <p className="text-xl text-text-secondary mb-3">世界首个人与分身共存的办公协作网络</p>
        <p className="text-lg text-accent font-medium mb-10">People quit. Clones don't.</p>
  ...
  return (
    <SlideShell>
      ...
      <p className="text-base text-text-secondary mb-10">
        不是代码 — 代码在 GitHub。不是文档 — 文档在 Notion。他带走的是三年的记忆和判断力。
      </p>
        ...
        <div className="bg-danger/5 border border-danger/15 rounded-xl p-6">
          ...
          <div className="space-y-3 text-[13px] text-text-secondary">
            <div className="flex items-start gap-2">
              ...
              <span>核心员工离职 = 三年记忆全部流失</span>
            </div>
            <div className="flex items-start gap-2">
              <X size={14} className="text-danger mt-0.5 shrink-0" />
              <span>新人 onboarding 3 个月，团队空转</span>
            </div>
            <div className="flex items-start gap-2">
              <X size={14} className="text-danger mt-0.5 shrink-0" />
              <span>决策依据、客户偏好、协作默契无法传承</span>
            </div>
            <div className="flex items-start gap-2">
              <X size={14} className="text-danger mt-0.5 shrink-0" />
              <span>每个 AI 工具都是孤岛，对话结束什么也没留下</span>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={d.label} className="flex items-baseline gap-3">
                <span className="text-xl font-bold text-accent tabular-nums w-20 shrink-0 text-right">
                  {d.num}
                </span>
...
function SlideInsight() {
  return (
    <SlideShell>
      ...
      <p className="text-base text-text-secondary mb-10">
        LinkedIn 连接人与人。Slack 连接人与团队。下一个是什么？
      </p>
  ...
  return (
    <SlideShell>
      ...
      <div className="bg-surface-1 border border-border rounded-xl p-5 text-center">
        <p className="text-[15px] font-bold text-text-primary">People quit. Clones don't.</p>
  ...
  return (
    <SlideShell>
      ...
      <p className="text-base text-text-secondary mb-8">
        不是一段对话记录，是一个完整的人 — 记住你的身份、记忆、人脉、知识、能力。
      </p>

      <div className="grid grid-cols-2 gap-6">
        <div className="bg-surface-2 border border-border rounded-xl p-6">
          <div className="font-mono text-[12px] text-text-secondary leading-[2]">
            <div className="text-text-primary font-medium mb-2 text-[13px]">
              分身的大脑 — 它记住的一切
            </div>
            <div>
              <span className="text-accent">├── .soul/</span> 身份、价值观、工作风格
            </div>
            <div>
              <span className="text-accent">├── contacts/</span> 人脉关系（每人一个 .md）
            </div>
            <div>
              <span className="text-accent">├── memory/</span> 决策、想法、偏好、事实
            </div>
            <div>
              <span className="text-accent">├── knowledge/</span> 知识库、竞品、市场
            </div>
            <div>
              <span className="text-accent">├── artifacts/</span> 产出物（报告、方案）
            </div>
            <div>
              <span className="text-accent">├── sessions/</span> 对话记录
            </div>
            <div>
              <span className="text-accent">├── automation/</span> 自动化规则
            </div>
            <div>
              <span className="text-accent">└── skills/</span> 能力定义（SKILL.md）
            </div>
          </div>
        </div>
        <div className="space-y-4">
          {[
            {
              icon: Globe,
              title: "IM-Native 入口",
              desc: "住在飞书/Slack/WhatsApp 里，零安装，加个 Bot 3 分钟上岗",
              color: "text-emerald-400",
            },
            {
              icon: Brain,
              title: "持续记忆",
              desc: "每次对话自动沉淀记忆，3 个月后的随口一说它也记得",
              color: "text-violet-400",
            },
            {
              icon: Zap,
              title: "数千种能力 + 自动化",
              desc: "写报告、做分析、监控竞品、定时推送 — 分身全天候工作",
              color: "text-amber-400",
            },
            {
              icon: Users,
              title: "分身网络协作",
              desc: "你的分身问同事的分身进度 → 自动更新 Sprint → 推送群聊",
              color: "text-cyan-400",
            },
          ].map((f) => (
...
function SlideMoat() {
  return (
    <SlideShell>
      ...
      <p className="text-base text-text-secondary mb-8">
        Manus 只做了 Skills 层 — 表层可以复制，深层不可能。
      </p>
      ...
      <div className="space-y-4">
        {[
          ...
          {
            layer: "深层",
            title: "Memory Lock-in + Network Effect",
...
function SlideMarket() {
  return (
    <SlideShell>
      ...
      <p className="text-base text-text-secondary mb-8">
        不是做"更好的 SaaS"，是 Agent 时代的新基础设施替代整个 SaaS 层。
      </p>
...
function SlideGTM() {
  return (
    <SlideShell>
      ...
      <p className="text-base text-text-secondary mb-8">
        不是 top-down 大客户销售。是 ProC 自费使用 → 同事看到 → 团队采购 → 企业买单。
      </p>
      ...
      <div className="grid grid-cols-3 gap-5 mb-8">
        {[
          ...
          },
          {
            ...
            who: "Enterprise",
            price: "Bottom-up 采购",
            desc: "多个团队在用 → IT 统一采购 → 企业级部署",
            color: "bg-accent",
          },
        ].map((p) => (
          <div key={p.phase} className="bg-surface-2 border border-border rounded-xl p-6">
            <div
              className={`inline-block px-2 py-0.5 rounded text-[10px] font-mono font-bold text-white mb-3 ${p.color}`}
            >
              {p.phase}
            </div>
      ...
      <div className="bg-surface-1 border border-border rounded-xl p-5">
        ...
        <div className="flex items-center justify-between gap-2 text-center">
          {[
            { id: "add-bot", label: "加 Bot", sub: "3 分钟上岗", color: "text-blue-400" },
            { id: "arrow-1", label: "→", sub: "", color: "text-text-muted" },
            { id: "daily-use", label: "日常使用", sub: "对话即操控", color: "text-violet-400" },
            { id: "arrow-2", label: "→", sub: "", color: "text-text-muted" },
            { id: "memory", label: "记忆沉淀", sub: "越用越懂你", color: "text-accent" },
            { id: "arrow-3", label: "→", sub: "", color: "text-text-muted" },
            { id: "visible", label: "群里可见", sub: "同事种草", color: "text-cyan-400" },
            { id: "arrow-4", label: "→", sub: "", color: "text-text-muted" },
            { id: "fission", label: "团队裂变", sub: "k > 1", color: "text-emerald-400" },
          ].map((s) =>
            s.sub === "" ? (
              <div key={s.id} className={`${s.color} text-lg`}>
                {s.label}
              </div>
            ) : (
              <div key={s.id} className="px-3 py-2 rounded-lg bg-surface-3">
  ...
  return (
    <SlideShell>
      ...
      <h2 className="text-3xl font-bold text-text-primary mb-2">
        不是更好的 Skills，是全新的基础设施
      </h2>
      <p className="text-base text-text-secondary mb-8">
        Manus 做了一些 Skills（PPT/Deep Research），在 OpenClaw
        背景下瞬间被消解。我们重建整套办公基础设施。
      </p>
...
function SlideVision() {
  return (
    <div className="w-full h-full flex items-center justify-center relative overflow-hidden">
      ...
      <div className="relative text-center max-w-3xl px-8">
        <div className="text-[11px] font-mono text-accent uppercase tracking-widest mb-6">
          09 · VISION
        </div>
        ...
        <p className="text-base text-text-secondary mb-8 max-w-xl mx-auto leading-relaxed">
          ...
          <br />
          <br />
          分身网络大规模运转后，我们拿到的数据是 OpenAI/Anthropic 永远拿不到的 — 不是对话数据，是
          <strong className="text-text-primary">工作闭环数据</strong>。
        </p>

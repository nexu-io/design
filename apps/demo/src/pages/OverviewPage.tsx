...
import { Brain, MessageSquare, Zap, Shield, Users, Heart, FileText, Sparkles, Bot, Target, Layers, Globe } from 'lucide-react'
    ...
    <PageShell>
      ...
      <Section title='产品定位'>
        <div className='bg-surface-2 border border-border rounded-lg p-6'>
          ...
          <p className='text-[13px] text-text-secondary mb-6 leading-relaxed'>
            不是 AI 助手，不是 workflow 工具，不是 agent builder。<br />
            是世界首个<strong className='text-text-primary'>人与分身共存的办公协作网络</strong> — 每个人有一个分身，分身记住你的一切，人走了分身留下，知识永不流失。
          </p>
      ...
      <Section title='设计信条'>
        <div className='grid grid-cols-2 gap-4'>
          {[
            { icon: Heart, title: '有温度的专业', desc: 'Linear 的克制 + 分身的温暖。暗色主题，琥珀色是分身"活着"的信号。' },
            { icon: Layers, title: '卡片即交互', desc: '6 种标准卡片是核心交互单元。所有操作产出可触碰、可交互的卡片。' },
            { icon: Zap, title: '会呼吸的 UI', desc: '分身有状态——在线、思考中、工作中。界面跟随分身状态，Avatar 有呼吸动画。' },
            { icon: Shield, title: '渐进式复杂度', desc: '第一眼极简（Session 入口），深入才看到更多（Scoped Session、任务系统、OKR）。' },
            { icon: Globe, title: '全球化就绪', desc: '中英双语，IM-native 入口适配多市场（飞书/Slack/WhatsApp/Telegram）。' },
            { icon: Brain, title: '分身心智模型', desc: '每个触点都强化「你有一个数字分身」。不说功能说感受，不说技术说关系，不说工具说同事。' },
          ].map(({ icon: Icon, title, desc }) => (
            <div key={title} className='bg-surface-2 border border-border rounded-lg p-5 flex gap-4'>
      ...
      <Section title='品牌声音'>
        <div className='bg-surface-2 border border-border rounded-lg p-5'>
          ...
          <div className='mt-5 pt-4 border-t border-border'>
            <div className='text-[11px] font-medium text-text-muted uppercase tracking-wider mb-3'>叙事框架（按受众）</div>
            <div className='grid grid-cols-2 gap-3'>
              {[
                { audience: '对用户', narrative: '你有一个分身，记住一切、帮你做事。人走了，分身留下。', color: 'border-l-emerald-500' },
                { audience: '对投资人', narrative: '龙虾的赛博办公室 — 不是 Skills 集合，是办公基础设施 + 分身网络。网络效应 + 记忆锁定 + 经验传承', color: 'border-l-violet-500' },
                { audience: '对开发者', narrative: '分身拥有完整的大脑 — 身份·记忆·知识·能力。人和 Agent 都能读写，没有翻译层。', color: 'border-l-blue-500' },
                { audience: '对团队市场', narrative: '每个人都有分身，分身帮你协作。员工走了，分身留下。知识永不流失。', color: 'border-l-cyan-500' },
              ].map(n => (
                <div key={n.audience} className={`p-3 bg-surface-3 rounded-lg border-l-2 ${n.color}`}>
                  <div className='text-[11px] font-medium text-text-primary mb-1'>{n.audience}</div>
                  <div className='text-[11px] text-text-secondary leading-relaxed'>{n.narrative}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <Section title='术语对照'>
        ...
        <div className='grid grid-cols-3 gap-3'>
          {[
            ...
            <div key={from} className='bg-surface-2 border border-border rounded-md p-3 flex items-center gap-2'>
      ...
      <Section title='产品架构概览'>
        <div className='bg-surface-2 border border-border rounded-lg p-5'>
          <div className='grid grid-cols-4 gap-4 mb-5'>
            {[
              { icon: Bot, label: '主线入口', desc: 'Session — 对话即操控一切', accent: 'text-accent' },
              { icon: FileText, label: '6 种卡片', desc: '文件·记忆·技能·自动化·协作·升级', accent: 'text-emerald-400' },
              { icon: Brain, label: '记忆系统', desc: '结构化记忆系统 — 越用越懂你', accent: 'text-violet-400' },
              { icon: Target, label: '团队协作', desc: '任务流·OKR·Sprint·分身网络', accent: 'text-cyan-400' },
            ].map(item => (
          ...
          <div className='p-4 bg-surface-3 rounded-xl font-mono text-[11px] text-text-secondary leading-relaxed'>
            ...
            <div className='grid grid-cols-2 gap-x-6 gap-y-0.5'>
              <div><span className='text-clone'>├── .soul/</span>          身份（我是谁、价值观、世界观）</div>
              <div><span className='text-clone'>├── contacts/</span>       人脉（每人一个 .md）</div>
              <div><span className='text-clone'>├── memory/</span>         记忆（决策、想法、偏好、事实）</div>
              <div><span className='text-clone'>├── knowledge/</span>      知识库（架构、参考、市场）</div>
              <div><span className='text-clone'>├── artifacts/</span>      产出物（PRD、报告、设计）</div>
              <div><span className='text-clone'>├── sessions/</span>       会话记录</div>
              <div><span className='text-clone'>├── automation/</span>     自动化规则（.yaml）</div>
              <div><span className='text-clone'>└── skills/</span>         技能定义（SKILL.md）</div>
            </div>
          </div>
        </div>
      </Section>

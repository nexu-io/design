      ...
      <Section title='Type Scale'>
        <div className='space-y-0'>
          {[
            ...
            { name: 'H4', size: '16px', lh: '24px', weight: '600', sample: '你的分身掌握了 5 项技能' },
            { name: 'Body', size: '14px', lh: '22px', weight: '400', sample: '分身记住了你的每一个想法、决策、偏好。用了 3 个月，它比你更记得你做过的事。' },
            { name: 'SM (IM)', size: '13px', lh: '20px', weight: '400', sample: '搞定了，你看看效果。我已经帮你连接了飞书日历和待办。' },
            { name: 'XS', size: '12px', lh: '18px', weight: '400', sample: '2 月 20 日 · 来自飞书对话' },
            { name: 'XXS', size: '11px', lh: '16px', weight: '500', sample: '专业版 · 在线 · 5 项技能' },
          ].map(t => (
            <div key={t.name} className='flex items-baseline gap-6 py-4 border-b border-border-subtle'>
              <div className='w-16 shrink-0'>
                <div className='text-[11px] font-mono text-accent'>{t.name}</div>
                <div className='text-[11px] text-text-muted'>{t.size}</div>
              </div>
              <div className='flex-1' style={{ fontSize: t.size, lineHeight: t.lh, fontWeight: t.weight }}>
                {t.sample}
              </div>
        ...
        <div className='grid grid-cols-4 gap-4'>
          {[
            ...
            <div key={w.weight} className='bg-surface-2 border border-border rounded-lg p-4'>
              <div className='text-2xl text-text-primary mb-2' style={{ fontWeight: w.weight }}>Aa 你好</div>
              <div className='text-xs text-text-secondary'>{w.name} — {w.weight}</div>
        ...
        <div className='flex gap-4'>
          {[
            ...
            <div key={r.name} className='text-center'>
              ...
              <div className='text-[11px] text-text-muted'>{r.px === 9999 ? '9999px' : `${r.px}px`}</div>
            </div>
          ))}
        </div>
      </Section>

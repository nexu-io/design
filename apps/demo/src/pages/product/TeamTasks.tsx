    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div
            className={`text-[13px] font-medium ${task.status === "archived" ? "text-text-muted line-through" : "text-text-primary"}`}
          >
            {task.title}
          </div>
          <div className="flex items-center gap-3 mt-1.5 flex-wrap">
            ...
            {task.tags.slice(0, 3).map((tag) => (
              <span
                key={tag}
                className="text-[9px] px-1.5 py-0.5 bg-surface-3 text-text-muted rounded"
              >
                {tag}
              </span>
            ))}
      ...
      {expanded && (
        <div className="border-t border-border px-4 py-3 space-y-3 bg-surface-0/50">
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            />
            {task.sprintId && (
              <MetaChip label="Sprint" value={task.sprintId} icon={Target} color="text-info" />
            )}
            {task.okrTitle && (
              <MetaChip
                label="OKR"
              ...
              <div className="space-y-1">
                {task.subtasks.map((sub) => (
                  <div key={sub.title} className="flex items-center gap-2 text-[12px]">
                    ...
                    <span
                      className={sub.done ? "text-text-muted line-through" : "text-text-primary"}
                    >
                      {sub.title}
                    </span>
                  </div>
            ...
            <div>
              ...
              <div className="space-y-1">
                {task.linkedFiles.map((f) => (
                  <div
                    key={f}
                    className="flex items-center gap-1.5 text-[11px] text-info hover:underline cursor-pointer"
                  >
                    <FileText size={11} /> {f}
                  </div>
                ))}
          ...
          {task.dependencies && task.dependencies.length > 0 && (
            <div className="flex items-center gap-2">
              ...
              {task.dependencies.map((d) => (
                <span
                  key={d}
                  className="text-[10px] px-1.5 py-0.5 bg-danger-subtle text-danger rounded font-mono"
                >
                  {d}
                </span>
              ))}
          ...
          <div className="flex items-center gap-2 pt-1">
            ...
            {task.executor !== "agent" && <ActionBtn icon={Bot} label="委托给 Agent" />}
            {task.status === "done" && <ActionBtn icon={Archive} label="归档" />}
              ...
              >
                ...
                <TableCell>
                  <div className="inline-flex items-center gap-1.5">
                    ...
                    <span className="text-[11px] text-text-secondary">{status.label}</span>
                  </div>
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="text-[12px] text-text-primary">{task.assignee}</div>
                      <div className={`flex items-center gap-1 text-[10px] ${executor.color}`}>
                        <executor.icon size={10} />
                        {executor.label}
                      </div>
                    </div>
                  </div>
                ...
                <TableCell>
                  <div className="min-w-[120px]">
                    <div className="mb-1 flex items-center justify-between text-[10px]">
                      ...
                      {task.comments > 0 ? (
                        <span className="text-text-muted">{task.comments} 评论</span>
                      ) : null}
                    </div>
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        <div className="flex items-center justify-between mb-2.5">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              type="button"
              size="inline"
              className="flex items-center gap-1 px-2.5 py-1.5 bg-accent text-accent-fg rounded-lg text-[11px] font-medium hover:bg-accent-hover transition-colors"
            >
              <Plus size={12} /> 新建任务
            </Button>
          </div>
              ...
              return (
                ...
                >
                  ...
                  {count !== undefined && count > 0 && (
                    <span
                      className={`ml-1 text-[9px] ${active ? "text-accent" : "text-text-muted"}`}
                    >
                      {count}
                    </span>
                  )}
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5">
        ...
        {filtered.length === 0 && (
          <div className="text-center py-12 text-text-muted text-[13px]">没有匹配的任务</div>
        )}
      </div>
    </div>
  );

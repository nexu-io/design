...
import {
  ...
  Globe, Check, Plus, ChevronRight, FolderOpen,
  Shield, Pencil, RefreshCw, Mail, Phone,
...
const CHANNELS: Channel[] = [
  ...
  {
    name: 'Email', icon: '📧', status: 'connected', desc: 'tom@refly.ai · IMAP/SMTP 已配置', badge: '零安装',
    category: 'native',
  ...
  {
    name: 'SMS / iMessage', icon: '💬', status: 'connected', desc: '+86 138****7890 · Twilio 接入', badge: '零安装',
    category: 'native',
  ...
  {
    name: 'WhatsApp', icon: '📱', status: 'pending', desc: '待绑定手机号 · Meta API', badge: '零安装',
    category: 'native',
    stats: null, recent: [],
  ...
  {
    name: '飞书', icon: '🐦', status: 'connected', desc: 'Bot 已上线 · WebSocket 活跃', badge: '主渠道',
    category: 'im',
  ...
  {
    name: 'Slack', icon: '💼', status: 'connected', desc: 'OAuth 已授权 · 3 个 Channel', badge: null,
    category: 'im',
    stats: { rounds: 523, files: 34, memories: 67, contacts: 3 },
    recent: [
      { type: 'file', label: '更新了竞品分析报告', time: '昨天' },
    ],
  },
  {
    name: 'Telegram', icon: '✈️', status: 'pending', desc: '待配置 Bot Token', badge: null,
    category: 'im',
    stats: null, recent: [],
  },
  {
    name: 'Discord', icon: '🎮', status: 'off', desc: '即将支持', badge: '敬请期待',
    category: 'im',
    stats: null, recent: [],
  },
  {
    name: '企业微信', icon: '🏢', status: 'off', desc: '即将支持', badge: '敬请期待',
    category: 'im',
    stats: null, recent: [],
  },
]
...
const CONTACTS = [
  {
    name: '张明', role: 'CTO', team: 'Engineering', file: 'contacts/张明-CTO.md',
    avatar: '👨‍💻', channel: '飞书', relation: '直属上级',
    notes: '偏好异步沟通，技术决策倾向稳定成熟方案，周三下午有固定 1on1',
    lastInteraction: '今天',
  },
  {
    name: '李薇', role: 'Product Manager', team: 'Product', file: 'contacts/李薇-PM.md',
    avatar: '👩‍💼', channel: '飞书', relation: '核心协作',
    notes: '需求文档偏好 Notion 格式，喜欢用数据说话，每周一提交周报',
    lastInteraction: '2 小时前',
  },
  {
    name: '王浩', role: '前端工程师', team: 'Engineering', file: 'contacts/王浩-前端.md',
    avatar: '🧑‍💻', channel: '飞书', relation: '同组同事',
    notes: 'React + TypeScript 专家，负责 Design System，代码审查很仔细',
    lastInteraction: '昨天',
  },
  {
    name: '刘芳', role: 'UI/UX 设计师', team: 'Design', file: 'contacts/刘芳-设计.md',
    avatar: '👩‍🎨', channel: 'Slack', relation: '跨组协作',
    notes: '偏好 Figma 评审，设计稿交付用 Notion，注重细节和动效',
    lastInteraction: '3 天前',
  },
  {
    name: '陈杰', role: '后端工程师', team: 'Engineering', file: 'contacts/陈杰-后端.md',
    avatar: '👨‍🔧', channel: '飞书', relation: '同组同事',
    notes: 'NestJS + PostgreSQL 负责人，接口文档写得好，喜欢 TDD',
    lastInteraction: '昨天',
  },
  {
    name: 'Alex Chen', role: 'Investor / Advisor', team: 'External', file: 'contacts/alex-investor.md',
    avatar: '🤵', channel: 'Email', relation: '投资人',
...
const CONTACT_GROUPS = [
  { name: 'Product Team', file: 'contacts/_groups/product-team.md', members: ['张明', '李薇', '王浩', '刘芳', '陈杰'], icon: '🏢' },
  { name: 'Engineering', file: 'contacts/_groups/engineering.md', members: ['张明', '王浩', '陈杰'], icon: '⚙️' },
  { name: 'Founders & Investors', file: 'contacts/_groups/founders.md', members: ['Alex Chen'], icon: '💼' },
]
...
const MEMORY_ENTRIES = [
  { id: 1, category: 'goals', content: '我是一名在上海的产品经理，其公司 refly.ai 正在构建基于 Openclaw 的数字员工基础设施，并刚完成了一轮顶级 VC 投资。', private: true, source: 'conversation', time: '2 天前' },
  { id: 2, category: 'ideas', content: '我认为当前很多人对 AI 的使用还停留在"电子宠物"阶段，但我更认同将 AI 视为"赛博合伙人"的协作理念。', private: false, source: 'conversation', time: '3 天前' },
  { id: 3, category: 'worldview', content: 'AI 的关键在于其行动力，即 AI 不应只停留在思考层面，而必须能映射到现实世界，产生实际的连接和改变。', private: true, source: 'extracted', time: '4 天前' },
  { id: 4, category: 'preferences', content: '我喜欢运动，特别是乒乓球和跑步，同时也是一位影视达人。', private: true, source: 'conversation', time: '5 天前' },
  { id: 5, category: 'habits', content: '我的心态似乎比较好，即使有工作压力通常也不会影响睡眠。', private: true, source: 'extracted', time: '5 天前' },
  { id: 6, category: 'tone', content: '简洁直接，偏技术术语，喜欢用类比解释复杂概念，中文为主但技术词用英文。', private: false, source: 'fine-tuned', time: '1 周前' },
  { id: 7, category: 'decisions', content: '用户决定使用 PostgreSQL + TypeORM 替代 MongoDB，理由是关系型查询更适合多模块 JOIN。', private: false, source: 'auto-extracted', time: '1 周前' },
  { id: 8, category: 'status', content: '当前 Sprint 主要任务：注册流程优化、Design System 改版、记忆系统设计。', private: false, source: 'proactive', time: '今天' },
  { id: 9, category: 'ideas', content: '观察到中心化 Agent IM bot（如 Manus 在 Telegram 上的 Personal Agent）被封，开始思考去中心化产品形态的风险。', private: false, source: 'conversation', time: '6 天前' },
  { id: 10, category: 'goals', content: '我有提升自己的目标，积极招聘增长/商业化/产品人才，打造世界级产品。', private: true, source: 'extracted', time: '1 周前' },
]

const MEMORY_CHANGELOG = [
  { action: 'merge', desc: '合并了「我是 Openclaw 的用户」相关记忆', time: '1 天前' },
  { action: 'extract', desc: '从 Tom 和 @biubiu暴雨 对话中提取了记忆点', source: '随便聊了聊', time: '2 天前' },
  { action: 'extract', desc: '从 Tom 和 @就爱喝无糖 对话中提取了记忆点', source: '随便聊了聊', time: '3 天前' },
...
const ARTIFACT_FOLDERS = [
  { folder: 'artifacts/prds/', count: 8, desc: '产品需求文档', files: ['注册流程优化.md', 'onboarding-redesign.md', 'onboarding-flowchart.svg'] },
  { folder: 'artifacts/research/', count: 14, desc: '竞品分析、市场调研', files: ['竞品注册流程对比.md', '市场调研数据.xlsx', '用户访谈-20260218.mp4'] },
  { folder: 'artifacts/code/', count: 5, desc: 'Schema、代码片段、API', files: ['schema-design.sql', 'oauth-callback.ts', 'api-spec.json'] },
  { folder: 'artifacts/designs/', count: 9, desc: '设计稿、Figma、UI 截图', files: ['landing-v2.figma', 'hero-mockup.png', 'onboarding-flow.svg', 'brand-guide.pdf'] },
  { folder: 'artifacts/reports/', count: 7, desc: 'Sprint 回顾、周报、指标', files: ['sprint-review-w8.md', 'monthly-metrics.xlsx', 'investor-deck.pptx'] },
  { folder: 'artifacts/media/', count: 6, desc: '产品 Demo、播客、素材', files: ['product-demo.mp4', 'podcast-ep3.mp3', 'logo-final.svg', 'screenshot-feeds.png'] },
]
...
const SOUL_FILES = [
  ...
  { path: '.soul/persona.md', label: '沟通风格', value: '简洁直接 · 偏技术 · 喜欢用类比', editable: true },
  { path: '.soul/persona.md', label: '语言', value: '中文为主 · 技术术语用英文', editable: true },
  { path: '.soul/persona.md', label: '工作习惯', value: '晚上效率高 · 喜欢先做难的 · 迭代速度快', editable: true },
]

const WORLDVIEW_FILES = [
  { path: '.soul/values.md', label: '产品哲学', value: '用户体验 > 功能数量，简单 > 复杂', icon: '🎯' },
  { path: '.soul/worldview.md', label: '技术价值观', value: '实用主义，不过度工程化，先跑起来再优化', icon: '⚡' },
  { path: '.soul/worldview.md', label: '决策原则', value: '"这让分身更懂用户了吗？" — 是就做，不是就砍', icon: '🧭' },
  { path: '.soul/values.md', label: '沟通准则', value: '直说结论，不说废话，有代码就直接写', icon: '💬' },
  { path: '.soul/worldview.md', label: '优先级框架', value: '用户感知价值 > 技术优雅度 > 扩展性', icon: '📊' },
]
...
const FEED_DETAILS: Record<string, FeedDetail> = {
  ...
  f3: {
    messages: [
      { from: 'clone', content: '注册流程优化方案已完成 ✅\n\n核心内容：\n• 方案 A：OAuth 优先（推荐）\n• 方案 B：飞书扫码\n• 包含完整流程图和技术方案\n• 预计注册转化率提升 40%' },
      { from: 'clone', content: '已将 PRD 存入 artifacts/prds/，同时更新了 OAuth 选型决策记录。需要我展开某个部分吗？' },
    ],
    fileOps: [
  ...
  f6: {
    messages: [
      ...
      { from: 'clone', content: '飞书日报已推送到「产品核心群」：\n\n今日工作摘要：\n• 完成注册流程 PRD\n• 竞品分析 - Linear/Notion/Cursor 对比\n• 数据库迁移 SQL 编写（3 个 migration）\n\n推送成功 ✅ 已有 3 人已读' },
    ],
    fileOps: [
      { action: 'create', path: 'artifacts/reports/daily-summary-0221.md' },
    ],
  },
  f7: {
    messages: [
      { from: 'clone', content: 'Deep Research 报告完成（AI Agent 市场分析）：\n\n📚 数据源：23 篇文献 + 5 个竞品\n📝 报告：8000 字深度分析\n\n核心发现：\n1. AI Agent 市场 2026 预计 $47B\n2. 个人 Agent 赛道尚无明确赢家\n3. "Agent Computer" 定位可建立差异化\n4. 文件系统 + 技能系统是关键壁垒' },
    ],
    fileOps: [
  ...
  f8: {
    messages: [
      ...
      { from: 'clone', content: '部署完成 ✅\n\n• 构建耗时：42s\n• 部署环境：Vercel Production\n• URL: https://nexu.app\n• Bundle size: 312 KB (gzipped)\n\n无错误，所有检查通过。' },
    ],
    fileOps: [
      { action: 'create', path: 'artifacts/reports/deploy-log-0220.md' },
    ],
  },
}
        ...
        {detail.fileOps.length > 0 && (
          <div className='space-y-1 pt-1'>
            ...
            {detail.fileOps.map((op, i) => {
              ...
              return (
                <div key={i} className='flex items-center gap-1.5 px-2 py-1.5 bg-surface-1 border border-border rounded-lg text-[11px]'>
                  ...
                  <span className='text-text-muted truncate ml-auto text-[10px] font-mono'>{parts.join('/')}/</span>
                </div>
              )
            })}
          </div>
...
function FeedsTab() {
  ...
  return (
    <div className='flex gap-0 items-stretch -mr-6'>
      {/* Feed list */}
      <div className={`space-y-4 transition-all duration-200 ${selectedFeed ? 'flex-1 min-w-0' : 'w-full'}`}>
        {/* Quick chat input */}
        <div className='bg-surface-1 border border-border rounded-xl p-4'>
          ...
          <div className='flex items-center gap-2 mt-2.5'>
            ...
            {['帮我写 PRD', '跑一下竞品监控', '今天做了什么'].map(q => (
              <button
                key={q}
                onClick={() => { setChatInput(q); navigate('/app/sessions') }}
          ...
          {FEED_ITEMS.map(item => {
            ...
            return (
              <div
                ...
                onKeyDown={e => { if (e.key === 'Enter') setSelectedFeed(isSelected ? null : item) }}
                ...
                <div className='flex items-start gap-3'>
                  ...
                  <div className='flex-1 min-w-0'>
                    <div className='flex items-center gap-2 mb-1'>
                      ...
                      <span className='text-[11px] text-text-muted ml-auto shrink-0'>{item.time}</span>
                    </div>
                    <h4 className='text-[13px] font-medium text-text-primary mb-0.5'>{item.title}</h4>
                    ...
                    <div className='flex items-center gap-3 mt-2'>
                      ...
                      <div className='flex-1' />
                      <span className={`text-[11px] text-accent font-medium flex items-center gap-1 transition-opacity ${
                        isSelected ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
                      }`}>
                        {isSelected ? '收起详情' : '查看详情'}
                        <ChevronRight size={11} className={`transition-transform ${isSelected ? 'rotate-180' : ''}`} />
                      </span>
                    </div>
                  </div>
                </div>
      ...
      {/* Detail panel */}
      {selectedFeed && (
        <div className='shrink-0 ml-4 sticky top-0 self-start' style={{ height: 'calc(100vh - 160px)' }}>
  ...
  return (
    <div className='bg-surface-2 border border-border rounded-xl hover:border-border-hover transition-colors overflow-hidden'>
      <div className='flex items-center gap-4 p-4'>
        <div className='text-2xl w-10 h-10 flex items-center justify-center bg-surface-3 rounded-lg shrink-0'>
          {ch.icon}
        </div>
        <div className='flex-1 min-w-0'>
          <div className='flex items-center gap-2'>
            ...
            {ch.badge && (
              <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${
                ch.badge === '主渠道' ? 'bg-clone/10 text-clone'
                : ch.badge === '零安装' ? 'bg-success-subtle text-success'
                : 'bg-surface-3 text-text-muted'
              }`}>{ch.badge}</span>
            )}
          </div>
          <div className='text-xs text-text-secondary mt-0.5'>{ch.desc}</div>
        </div>
        <div className='flex items-center gap-2'>
          {ch.status === 'connected' && (
            ...
            <div className='pt-2 border-t border-border-subtle space-y-1'>
              {ch.recent.map((r, i) => (
                <div key={i} className='flex items-center gap-2 text-[11px] group cursor-pointer'>
                  ...
                  <span className='text-text-secondary group-hover:text-text-primary transition-colors'>{r.label}</span>
    ...
    <div className='space-y-6'>
      ...
      <div>
        <div className='flex items-center gap-2 mb-3'>
          ...
          <span className='text-[10px] text-success bg-success-subtle px-1.5 py-0.5 rounded-full font-medium'>零安装</span>
        </div>
        <div className='text-[12px] text-text-secondary mb-3 pl-7'>
          无需下载任何 App — 每个手机都有短信和邮件。发一条消息就能启动分身工作。
        </div>
        <div className='space-y-3'>
          {nativeChannels.map(ch => <ChannelCard key={ch.name} ch={ch} />)}
        </div>
      </div>
      ...
      <div>
        ...
        <div className='space-y-3'>
          {imChannels.map(ch => <ChannelCard key={ch.name} ch={ch} />)}
        </div>
      </div>
  ...
  return (
    <div className='space-y-6'>
      ...
      <div>
        ...
        <div className='flex flex-wrap gap-2'>
          {CONTACT_GROUPS.map(g => (
            <div key={g.name} className='flex items-center gap-2 px-3 py-2 bg-surface-2 border border-border rounded-lg hover:border-border-hover transition-colors cursor-pointer'>
              <span>{g.icon}</span>
      ...
      <div>
        ...
        <div className='space-y-2'>
          {CONTACTS.map(c => (
            <div key={c.name} className='p-3 bg-surface-2 border border-border rounded-lg hover:border-border-hover transition-colors group'>
              <div className='flex items-center gap-3'>
                <div className='w-9 h-9 rounded-full bg-surface-3 flex items-center justify-center text-lg shrink-0'>
                  {c.avatar}
                </div>
                <div className='flex-1 min-w-0'>
                  <div className='flex items-center gap-2'>
                    <span className='text-[13px] font-medium text-text-primary'>{c.name}</span>
                    <span className='text-[10px] px-1.5 py-0.5 bg-surface-3 rounded text-text-muted'>{c.role}</span>
                    <span className='text-[10px] px-1.5 py-0.5 bg-surface-3 rounded text-text-muted'>{c.relation}</span>
                  </div>
                  <div className='text-[11px] text-text-secondary mt-0.5'>{c.notes}</div>
                </div>
                <div className='flex flex-col items-end gap-1 shrink-0'>
                  <div className='flex items-center gap-1 text-[10px] text-text-muted'>
                    {c.channel === 'Email' ? <Mail size={10} /> : c.channel === 'Slack' ? <MessageSquare size={10} /> : <Phone size={10} />}
                    {c.channel}
                  </div>
                  <span className='text-[10px] text-text-muted'>{c.lastInteraction}</span>
                </div>
              </div>
              <div className='mt-2 pt-2 border-t border-border-subtle flex items-center justify-between opacity-0 group-hover:opacity-100 transition-opacity'>
                <span className='text-[10px] text-text-muted font-mono'>{c.file}</span>
                <button className='p-0.5 text-text-muted hover:text-text-secondary'>
                  <Pencil size={10} />
                </button>
              </div>
            </div>
...
function MemoryTab() {
  ...
  const filtered = activeCat === 'all'
    ? MEMORY_ENTRIES
    : MEMORY_ENTRIES.filter(m => m.category === activeCat)
  ...
  return (
    <div className='space-y-4'>
      ...
      <div className='flex items-center gap-1.5 overflow-x-auto pb-1'>
        {MEMORY_CATEGORIES.map(cat => (
          ...
          >
            ...
            {cat.label}
            <span className={`text-[10px] tabular-nums ${activeCat === cat.id ? 'text-accent-fg/70' : 'text-text-muted'}`}>
              {cat.count}
            </span>
          </button>
        ))}
      ...
      >
        ...
        <ChevronRight size={14} className={`text-text-muted transition-transform ${showChangelog ? 'rotate-90' : ''}`} />
      </button>
      ...
      <div className='space-y-2'>
        {filtered.map(m => {
          ...
          return (
            <div key={m.id} className='p-4 bg-surface-2 border border-border rounded-xl hover:border-border-hover transition-colors group'>
              <div className='flex items-center gap-2 mb-2'>
                <span className='text-sm'>{cat?.icon}</span>
                <span className='text-[12px] font-medium text-clone'>{cat?.label}</span>
                <div className='ml-auto flex items-center gap-2'>
                  {m.source === 'fine-tuned' && (
                    <span className='text-[9px] px-1.5 py-0.5 bg-clone/10 text-clone rounded-full'>微调</span>
                  )}
                  {m.source === 'auto-extracted' && (
                    <span className='text-[9px] px-1.5 py-0.5 bg-info-subtle text-info rounded-full'>自动提取</span>
                  )}
                  {m.source === 'proactive' && (
                    <span className='text-[9px] px-1.5 py-0.5 bg-success-subtle text-success rounded-full'>主动更新</span>
                  )}
    ...
    <div className='space-y-4'>
      ...
      <div className='space-y-2'>
        {ARTIFACT_FOLDERS.map(t => (
          <div key={t.folder} className='p-3 bg-surface-2 border border-border rounded-lg hover:border-border-hover transition-colors group'>
            ...
            <div className='pl-7 flex flex-wrap gap-1'>
              {t.files.map(f => (
                <span key={f} className='text-[10px] px-1.5 py-0.5 bg-surface-3 rounded text-text-muted font-mono'>
                  {f}
                </span>
              ))}
            </div>
    ...
    <div className='space-y-4'>
      ...
      <div className='text-center'>
        ...
        <div className='mt-3 px-2'>
          ...
          <div className='text-[9px] text-text-muted mt-1'>多聊天多"喂养"，对齐率越高分身越懂你</div>
        </div>
      </div>
      ...
      <div>
        <div className='flex items-center gap-1 mb-2'>
          ...
          <span className='text-[11px] font-medium text-text-secondary uppercase tracking-wider'>人设特质</span>
        </div>
        <div className='space-y-1.5'>
          {SOUL_FILES.map((f, i) => (
            <div key={i} className='flex items-start gap-2 p-2 bg-surface-2 border border-border rounded-lg group hover:border-border-hover transition-colors'>
              <div className='text-[10px] text-text-muted w-14 shrink-0 pt-0.5 uppercase tracking-wider'>{f.label}</div>
      ...
      <div>
        <div className='flex items-center gap-1 mb-2'>
          ...
          <span className='text-[11px] font-medium text-text-secondary uppercase tracking-wider'>世界观</span>
        </div>
        <div className='space-y-1.5'>
          {WORLDVIEW_FILES.slice(0, 3).map(w => (
            <div key={w.label} className='flex items-start gap-2 p-2 bg-surface-2 border border-border rounded-lg group hover:border-border-hover transition-colors'>
      ...
      <div className='pt-2 border-t border-border'>
        <div className='text-[10px] text-text-muted font-mono text-center'>~/.soul/identity.md · persona.md · worldview.md</div>
      </div>
    </div>
  )
}
  ...
  return (
    <div className='space-y-3'>
      {WORLDVIEW_FILES.map(w => (
        <div key={w.label} className='p-4 bg-surface-2 border border-border rounded-xl hover:border-border-hover transition-colors group'>
          <div className='flex items-center gap-2 mb-2'>
            <span className='text-lg'>{w.icon}</span>
            <span className='text-sm font-medium text-text-primary'>{w.label}</span>
            <span className='text-[10px] text-text-muted font-mono ml-auto opacity-0 group-hover:opacity-100 transition-opacity'>{w.path}</span>
...
export default function CloneBuilderPage() {
  ...
  const [activeTab, setActiveTab] = useState<TabId>(tabFromUrl && TABS.some(t => t.id === tabFromUrl) ? tabFromUrl : 'feeds')

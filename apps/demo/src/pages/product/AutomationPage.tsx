...
import {
  Plus, Clock, Bell, Brain, Zap, Database,
  MoreHorizontal, Play, Pause, Pencil,
  Calendar, Mail, FileText, Users,
  CheckCircle, AlertCircle, ToggleLeft, ToggleRight,
  Sparkles, BarChart3, Shield,
...
const AUTOMATION_TEMPLATES = [
  { name: '每日邮件摘要', desc: '汇总过去 24h 未读邮件，按发件人和优先级分类', icon: Mail, color: 'bg-info-subtle text-info' },
  { name: '竞品监测', desc: '每周扫描竞品动态、产品更新、融资消息', icon: Search, color: 'bg-clone/10 text-clone' },
  { name: '周五 Sprint 回顾', desc: '汇总本周完成 vs 计划任务，计算 velocity', icon: BarChart3, color: 'bg-success-subtle text-success' },
  { name: '安全自审', desc: '每晚 AI 审查系统日志、权限配置、异常行为检测', icon: Shield, color: 'bg-error-subtle text-error' },
  { name: '联系人健康扫描', desc: '每周扫描联系人，标记 >30 天未互动的关键人脉', icon: Users, color: 'bg-role-ops/10 text-role-ops' },
  { name: '行动项追踪', desc: '每天 3 次检查 action items 完成情况，追踪对方承诺', icon: CheckCircle, color: 'bg-success-subtle text-success' },
  { name: '知识库自动整理', desc: '新入库内容去重、合并、打标签、生成每周摘要', icon: Database, color: 'bg-role-designer/10 text-role-designer' },
  { name: '增长指标日报', desc: '每日汇总核心指标：DAU、转化率、留存、社交互动', icon: TrendingUp, color: 'bg-warning-subtle text-warning' },
  { name: '灵感 → 方案', desc: '新想法入库后自动搜索关联历史、评估可行性、生成方案', icon: Sparkles, color: 'bg-clone/10 text-clone' },
]

const SCHEDULED_TASKS = [
  {
    id: 6, name: '今日战况复盘', cron: '每天 22:00',
    desc: '扫荡信息流，评论互动，筛选有价值内容，生成战况报告发送到飞书', channel: '飞书',
    enabled: true, lastRun: '今天 22:00', icon: Brain,
    isHighlight: true,
    stats: '今日：扫荡 7,660 条 · 评论 21 次 · 发现 3 个潜在目标',
  },
  {
    id: 1, name: '早间日报', cron: '每天 08:30',
    desc: '汇总日程、待办、紧急事项，发送到飞书', channel: '飞书',
    enabled: true, lastRun: '今天 08:30', icon: Calendar,
  },
  {
    id: 8, name: 'Action Item 完成检查', cron: '每天 10:00 / 15:00 / 20:00',
    desc: '检查会议行动项完成情况，追踪对方承诺，未完成的主动提醒', channel: '飞书',
    enabled: true, lastRun: '今天 15:00', icon: CheckCircle,
    stats: '今日：检查 7 项 · 已完成 3 · 追踪对方 4 · 到期 1',
  },
  {
    id: 7, name: '记忆周报', cron: '每周日 20:00',
    desc: '复盘本周新增记忆、碎片聚合为结构化洞察、对齐率变化', channel: '飞书',
    enabled: true, lastRun: '上周日 20:00', icon: Database,
    stats: '本周：新增 23 条记忆 · 聚合 5 条碎片为洞察 · 对齐率 +3%',
  },
  {
    id: 2, name: '周五回顾', cron: '每周五 17:00',
    desc: '总结本周完成事项、未完成任务、下周规划', channel: '飞书',
    enabled: true, lastRun: '上周五 17:00', icon: FileText,
  },
  {
    id: 3, name: 'TODO 提醒', cron: '每天 10:00 / 15:00',
    desc: '检查临期和过期的待办事项并提醒', channel: '飞书',
    enabled: true, lastRun: '今天 10:00', icon: Bell,
  },
  {
    id: 9, name: '联系人健康扫描', cron: '每周一 09:00',
    desc: '扫描联系人互动频率，标记 >30 天未联系的重要人脉，生成关系报告', channel: '飞书',
    enabled: true, lastRun: '上周一 09:00', icon: Users,
    stats: '上周：扫描 24 人 · 标记 6 人需关注 · 2 人流失风险',
  },
  {
    id: 10, name: '安全自审', cron: '每晚 03:00',
    desc: 'AI 多视角审查系统运行日志、权限配置、异常行为检测，发现问题立即告警', channel: '飞书',
    enabled: true, lastRun: '今天 03:00', icon: Shield,
  },
  {
    id: 5, name: '邮件摘要', cron: '每 30 分钟（工作时段）',
    desc: '扫描未读邮件，过滤营销/冷推销，保留有价值对话并摘要', channel: '飞书',
    enabled: false, lastRun: '从未运行', icon: Mail,
  },
]

const PROACTIVE_RULES = [
  {
    id: 1, name: '灵感关联', trigger: '用户记录新想法时',
    action: '自动搜索相关历史记忆，发现关联并主动推送', enabled: true,
    example: '"你 3 天前说的 X 跟这个想法很像，要合并思考吗？"',
  },
  {
    id: 2, name: '截止日预警', trigger: '待办距截止 < 24h',
    action: '主动提醒用户并建议优先处理', enabled: true,
    example: '"数据库迁移今天到期，建议优先处理。需要帮你拆解步骤吗？"',
  },
  {
    id: 3, name: '上下文衔接', trigger: '新对话开始时',
    action: '自动加载最近相关记忆和未完成任务', enabled: true,
    example: '"你昨天让我帮你写的 PRD 还没完成，要继续吗？"',
  },
  {
    id: 4, name: '知识沉淀', trigger: '对话中出现重要决策',
    action: '自动提取并保存到记忆系统，更新分身认知', enabled: true,
    example: '自动保存：用户决定使用 PostgreSQL 替代 MongoDB',
  },
  {
    id: 5, name: '对方承诺追踪', trigger: '会议中对方承诺了行动项',
    action: '标记为"waiting on"，到期未完成时提醒用户跟进', enabled: true,
    example: '"王浩承诺今天提交飞书扫码 demo，但还没更新。需要帮你发消息跟进吗？"',
  },
  {
    id: 6, name: '跨模块智能关联', trigger: '正在做 A 任务时，B 模块有相关信息',
    action: '主动插入关联信息，打通模块间数据', enabled: true,
    example: '"你在写定价方案，刘强上次聊过类似话题——他关注单位经济模型。"',
  },
  {
    id: 7, name: '自动化推荐', trigger: '用户重复做同一件事 3+ 次',
    action: '分身主动推荐自动化规则，用户一键确认', enabled: true,
    example: '"你每周一上午都在整理上周 Todo，要我帮你自动化吗？"',
  },
  {
    id: 8, name: '依赖风险检测', trigger: '团队成员任务进度变更时',
    action: '检测跨成员任务依赖风险，自动推送对齐请求卡片到 IM', enabled: true,
    example: '"张三的前端集成依赖李四的 Gateway，后者进度偏慢，建议提前对齐。"',
  },
  {
    id: 9, name: '站会自动化', trigger: '每日 09:00',
    action: '汇总团队分身任务进度，生成站会 IM 卡片推送到群', enabled: true,
...
const ACTIVITY_LOGS = [
  ...
  { time: '08:32', type: 'proactive', name: '灵感关联', status: 'success', detail: '发现 2 条相关记忆' },
  { time: '10:00', type: 'schedule', name: 'TODO 提醒', status: 'success', detail: '提醒 3 条待办' },
  { time: '10:15', type: 'proactive', name: '截止日预警', status: 'warning', detail: '1 条任务今日到期' },
  { time: '11:30', type: 'proactive', name: '知识沉淀', status: 'success', detail: '自动保存 1 条决策' },
]
...
const AUTOMATION_RUN_LOGS: Record<number, RunLog[]> = {
  6: [
    { time: '今天 22:00', status: 'success', summary: '扫荡 7,660 条 · 评论 21 次 · 发现 3 个目标' },
    { time: '昨天 22:00', status: 'success', summary: '扫荡 5,320 条 · 评论 15 次 · 发现 1 个目标' },
    { time: '前天 22:00', status: 'success', summary: '扫荡 6,100 条 · 评论 18 次 · 发现 2 个目标' },
    { time: '2/18 22:00', status: 'warning', summary: '扫荡 4,200 条 · 评论 12 次 · API 限流 1 次' },
  ],
  1: [
  ...
  8: [
    { time: '今天 15:00', status: 'success', summary: '检查 7 项 · 已完成 3 · 追踪对方 4 · 王浩 demo 到期' },
    { time: '今天 10:00', status: 'success', summary: '检查 7 项 · 已完成 1 · 新增对方承诺 2' },
    { time: '昨天 20:00', status: 'warning', summary: '1 项过期：数据库 Schema 初稿（陈杰）' },
  ],
  7: [
    { time: '上周日 20:00', status: 'success', summary: '新增 23 条记忆 · 聚合 5 条碎片 · 默契度 +3%' },
  ...
  9: [
    { time: '上周一 09:00', status: 'success', summary: '扫描 24 人 · 6 人需关注 · 2 人流失风险 · 已发提醒' },
    { time: '2/10 09:00', status: 'success', summary: '扫描 22 人 · 3 人需关注 · 全部健康度正常' },
  ],
  10: [
    { time: '今天 03:00', status: 'success', summary: '审查 12 个 automation 日志 · 无异常 · 权限正常' },
    { time: '昨天 03:00', status: 'success', summary: '审查完成 · 发现 1 条建议：优化 API 限流策略' },
    { time: '前天 03:00', status: 'warning', summary: '检测到 1 个权限异常：邮件 skill 请求了写权限' },
  ],
}
...
const PROACTIVE_RUN_LOGS: Record<number, RunLog[]> = {
  ...
  5: [
    { time: '今天 16:30', status: 'warning', summary: '王浩承诺今天提交 demo，但截至 16:30 未更新' },
    { time: '昨天 10:00', status: 'success', summary: '陈杰已提交 Schema 初稿，标记为已完成' },
  ],
  6: [
    { time: '今天 14:20', status: 'success', summary: '你在写定价方案时，关联了刘强上次讨论的单位经济模型' },
    { time: '昨天 16:00', status: 'success', summary: '在竞品分析中，插入了知识库中 3 天前保存的 Cursor 资料' },
  ],
  7: [
    { time: '今天 09:00', status: 'success', summary: '检测到你本周 3 次手动整理 Todo，推荐自动化规则' },
        ...
        <div className='flex-1 overflow-y-auto min-h-0'>
          ...
          {logs.length > 0 ? (
            <div className='px-4 pb-3 space-y-1.5'>
              {logs.map((log, i) => (
                <div key={i} className='flex items-start gap-2.5 p-2.5 bg-surface-2 border border-border rounded-lg'>
    ...
    <div className='space-y-3'>
      ...
      <div className='mb-6'>
        ...
        <div className='grid grid-cols-3 gap-2'>
          {AUTOMATION_TEMPLATES.map(t => (
            ...
            >
              <div className={`w-7 h-7 rounded-lg ${t.color.split(' ')[0]} flex items-center justify-center mb-2`}>
                <t.icon size={14} className={t.color.split(' ')[1]} />
              </div>
              <div className='text-[12px] font-medium text-text-primary mb-0.5'>{t.name}</div>
              <div className='text-[10px] text-text-muted leading-relaxed line-clamp-2'>{t.desc}</div>
            </button>
          ))}
        </div>
      </div>
      ...
      {SCHEDULED_TASKS.map(task => (
        <div
          ...
          role='button'
          tabIndex={0}
          onClick={() => onSelectItem({ type: 'schedule', id: task.id, name: task.name, icon: task.icon, enabled: task.enabled })}
          onKeyDown={e => { if (e.key === 'Enter') onSelectItem({ type: 'schedule', id: task.id, name: task.name, icon: task.icon, enabled: task.enabled }) }}
        ...
        >
          ...
          <div className='flex items-center gap-2 shrink-0'>
            {task.enabled ? (
              <button onClick={e => e.stopPropagation()} className='p-1.5 rounded-md hover:bg-surface-3 text-success transition-colors'>
                <Pause size={14} />
              </button>
            ) : (
              <button onClick={e => e.stopPropagation()} className='p-1.5 rounded-md hover:bg-surface-3 text-text-muted transition-colors'>
                <Play size={14} />
              </button>
            )}
            <button onClick={e => e.stopPropagation()} className='p-1.5 rounded-md hover:bg-surface-3 text-text-muted transition-colors'>
...
function ProactiveTab({ onSelectItem }: { onSelectItem: (item: AutomationDetail) => void }) {
  return (
    <div className='space-y-3'>
      <div className='flex items-center gap-3 p-4 bg-clone/5 border border-clone/20 rounded-xl mb-4'>
        <Brain size={16} className='text-clone shrink-0' />
      ...
      {PROACTIVE_RULES.map(rule => (
        <div
          ...
          role='button'
          tabIndex={0}
          onClick={() => onSelectItem({ type: 'proactive', id: rule.id, name: rule.name, icon: Zap, enabled: rule.enabled })}
          onKeyDown={e => { if (e.key === 'Enter') onSelectItem({ type: 'proactive', id: rule.id, name: rule.name, icon: Zap, enabled: rule.enabled }) }}
        ...
        >
          <div className='flex items-center justify-between mb-2'>
            ...
            <button onClick={e => e.stopPropagation()} className='text-text-muted hover:text-text-secondary transition-colors'>
              {rule.enabled ? (
          ...
          <div className='text-xs text-text-secondary'>
            <span className='text-text-muted'>触发：</span>{rule.trigger}
          </div>
          <div className='text-xs text-text-secondary mt-1'>
            <span className='text-text-muted'>动作：</span>{rule.action}
          </div>
  ...
  return (
    <div className='space-y-1'>
      ...
      {ACTIVITY_LOGS.map((log, i) => (
        <div key={i} className='flex items-center gap-3 px-3 py-2.5 bg-surface-2 border border-border rounded-lg text-[13px]'>
          <span className='w-12 text-text-muted font-mono text-xs'>{log.time}</span>
          <span className='w-14'>
            {log.type === 'schedule' ? (
              <span className='text-[10px] px-1.5 py-0.5 bg-info-subtle text-info rounded'>定时</span>
            ) : (
  ...
  return (
    <div className='h-full overflow-y-auto'>
      <div className='max-w-3xl mx-auto px-6 py-8'>
        ...
        {/* Bottom note */}
        <div className='flex items-center gap-2 mt-8 p-3 bg-surface-2 border border-border rounded-lg text-xs text-text-secondary'>

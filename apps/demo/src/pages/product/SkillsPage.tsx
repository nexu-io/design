...
import {
  Search, Plus, Star, Download, Shield,
  Code, FileText, BarChart3, Megaphone,
  Server, Wrench, Lock,
  Check,
  Sparkles, Users, Building,
  MessageSquare, Workflow, X, Send,
...
const INSTALLED_SKILLS: Skill[] = [
  { name: 'Memory & Notes', desc: '自动记忆管理，知识沉淀', icon: FileText, author: 'nexu', installs: '12.4k', certified: true, installed: true, category: 'base' },
  { name: 'Task Manager', desc: '待办创建、追踪、提醒', icon: BarChart3, author: 'nexu', installs: '11.2k', certified: true, installed: true, category: 'base' },
  { name: 'Daily Digest', desc: '每日汇总日程 + 待办 + 要事', icon: FileText, author: 'nexu', installs: '9.8k', certified: true, installed: true, category: 'base' },
  { name: 'Web Research', desc: '联网搜索 + 信息整理', icon: Search, author: 'nexu', installs: '10.1k', certified: true, installed: true, category: 'base' },
  { name: 'Code Automation', desc: '代码生成、PR、自动化开发', icon: Code, author: 'nexu', installs: '8.3k', certified: true, installed: true, category: 'developer' },
]

const FEATURED_SKILLS: { title: string; badge: string; desc: string; color: string }[] = [
  { title: 'Advisory Board (Committee)', badge: 'SKILLS WE LOVE', desc: 'Multiple expert agents debate and produce actionable insights.', color: 'bg-clone/10' },
  { title: 'Contact Intelligence + CRM', badge: 'NEW', desc: 'AI-powered relationship management with health scoring.', color: 'bg-info-subtle' },
  { title: 'Zero-friction Knowledge Ingestion', badge: 'NEW', desc: 'Drop links, PDFs, screenshots — auto-process and organize.', color: 'bg-success-subtle' },
]

const RECOMMENDED_SKILLS: Skill[] = [
  { name: 'Contact Intelligence', desc: '联系人健康评分、互动分析、关系管理、主动提醒', icon: Users, author: 'nexu', installs: '7.2k', certified: true, installed: false, category: 'base' },
  { name: 'Meeting Action Tracker', desc: '会议 action item 提取 + mine/theirs 分类 + 完成追踪', icon: FileText, author: 'nexu', installs: '6.1k', certified: true, installed: false, category: 'productivity' },
  { name: 'Advisory Board', desc: '多 Agent 并行分析 → 辩论 → 综合建议 (Committee Technique)', icon: Building, author: 'nexu', installs: '3.8k', certified: true, installed: false, category: 'founder' },
  { name: 'Knowledge Ingester', desc: '零摩擦知识入库：链接/PDF/截图 → 自动抓取、摘要、向量化', icon: FileText, author: 'nexu', installs: '5.6k', certified: true, installed: false, category: 'base' },
  { name: 'Content Pipeline', desc: '灵感 → 全网搜索 → 去重 → 大纲 → 任务卡片', icon: Megaphone, author: 'nexu', installs: '4.5k', certified: true, installed: false, category: 'marketing' },
  { name: 'Security Auditor', desc: '多视角审查系统安全，自我进化规则，异常立即告警', icon: Shield, author: 'nexu', installs: '3.2k', certified: true, installed: false, category: 'devops' },
  { name: 'PRD Generator', desc: '需求文档生成与迭代', icon: FileText, author: 'nexu', installs: '6.7k', certified: true, installed: false, category: 'product' },
  { name: 'GitHub Digest', desc: '每日 PR/Issue 摘要，代码审查提醒', icon: Code, author: 'community/dev-tools', installs: '4.1k', certified: true, installed: false, category: 'developer' },
  { name: 'Notion Sync', desc: '双向同步 Notion 数据库和页面', icon: FileText, author: 'community/alex', installs: '3.2k', certified: false, installed: false, category: 'integration' },
  { name: 'Email Classifier', desc: '邮件智能分类 + 过滤营销/冷推销 + 优先级标注', icon: FileText, author: 'community/inbox-zero', installs: '1.9k', certified: false, installed: false, category: 'productivity' },
  { name: 'Investor Update', desc: '自动生成投资人周报 + 数据可视化', icon: Building, author: 'community/startups', installs: '1.5k', certified: false, installed: false, category: 'business' },
  { name: 'Deploy Checklist', desc: '部署前自动检查清单，CI/CD 集成', icon: Server, author: 'community/devops', installs: '2.1k', certified: true, installed: false, category: 'devops' },
]
...
const SKILL_DETAILS: Record<string, {
  ...
  'Web Research': {
    ...
    version: '2.0.0',
    updated: '2026-02-21',
    skillMd: '# Web Research\n\ntriggers: ["搜索", "查一下", "调研"]\ntools: [web_search, url_fetch]\n\n## Instructions\n联网搜索相关信息...',
  },
  'Code Automation': {
    longDesc: '代码生成、PR 创建、自动化开发任务。支持读取项目上下文、生成代码片段、创建 PR、代码审查摘要。',
    triggers: ['代码生成请求', 'PR 相关操作'],
    tools: ['code_write', 'github_api', 'file_read', 'file_write'],
    auth: ['GitHub OAuth'],
    version: '1.3.0',
    updated: '2026-02-17',
    skillMd: '# Code Automation\n\ntriggers: ["写代码", "生成", "PR"]\ntools: [code_write, github_api]\n\n## Instructions\n根据用户需求生成代码...',
  },
  'PRD Generator': {
    longDesc: '需求文档生成与迭代。基于用户描述自动生成结构化 PRD，支持竞品参考、用户故事、优先级排序。',
...
function SkillDetailPanel({ skill, onClose }: { skill: Skill; onClose: () => void }) {
  ...
  return (
    <div className='fixed inset-0 z-50 flex justify-end'>
      ...
      <div className='relative w-[420px] h-full bg-surface-1 border-l border-border shadow-2xl flex flex-col overflow-hidden animate-slide-in-right'>
        <div className='flex items-start gap-4 p-6 border-b border-border'>
          ...
          <button onClick={onClose} className='p-1 rounded-md hover:bg-surface-3 text-text-muted transition-colors shrink-0'>
            <X size={16} />
          </button>
        </div>
        ...
        <div className='flex-1 overflow-y-auto'>
          ...
          <div className='px-6 py-4 border-b border-border'>
            <h3 className='text-[12px] font-medium text-text-muted uppercase tracking-wider mb-2'>描述</h3>
          ...
          {detail && (
            <div className='px-6 py-4 border-b border-border'>
              <h3 className='text-[12px] font-medium text-text-muted uppercase tracking-wider mb-2'>触发条件</h3>
          ...
          {detail && (
            <div className='px-6 py-4 border-b border-border'>
              <h3 className='text-[12px] font-medium text-text-muted uppercase tracking-wider mb-2'>使用工具</h3>
              <div className='flex flex-wrap gap-1.5'>
                {detail.tools.map(t => (
                  <span key={t} className='px-2 py-1 bg-surface-3 rounded text-[11px] font-mono text-text-secondary'>
                    {t}
                  </span>
                ))}
              </div>
          ...
          {detail && detail.auth.length > 0 && (
            <div className='px-6 py-4 border-b border-border'>
              <h3 className='text-[12px] font-medium text-text-muted uppercase tracking-wider mb-2'>需要授权</h3>
              <div className='space-y-1.5'>
                {detail.auth.map((a, i) => (
                  <div key={i} className='flex items-center gap-2 text-[12px]'>
          ...
          {detail && (
            <div className='px-6 py-4'>
              <div className='flex items-center gap-2 mb-2'>
                ...
                <h3 className='text-[12px] font-medium text-text-muted uppercase tracking-wider'>SKILL.md</h3>
                <span className='text-[10px] text-text-muted font-mono ml-auto'>~/clone/skills/{skill.name.toLowerCase().replace(/\s+/g, '-')}/</span>
              </div>
...
function ChatCreatorModal({ onClose }: { onClose: () => void }) {
  ...
  const chatMessages = [
    ...
    { from: 'system' as const, content: '明白了！让我帮你拆解：\n\n📌 技能名称：GitHub PR Digest\n⏰ 触发条件：每天 09:00\n🔧 调用工具：GitHub API\n📄 输出格式：Markdown 摘要\n\n我来生成 SKILL.md...' },
  ]
        ...
        <div className='flex-1 overflow-y-auto p-4 space-y-3'>
          ...
          <div className='p-3 bg-surface-3/60 border border-border/50 rounded-xl'>
            <div className='flex items-center gap-2 mb-2'>
              ...
              <span className='text-[11px] font-mono text-text-secondary'>~/clone/skills/github-pr-digest/SKILL.md</span>
            </div>
...
function WorkflowEditorModal({ onClose }: { onClose: () => void }) {
  const NODES = [
    { id: 'trigger', label: '触发', desc: '每天 09:00', icon: Zap, x: 60, y: 40, color: 'text-warning' },
    { id: 'fetch', label: 'GitHub API', desc: '获取 Open PRs', icon: Code, x: 220, y: 40, color: 'text-info' },
    { id: 'filter', label: '过滤', desc: '只看 my team', icon: GitBranch, x: 380, y: 40, color: 'text-role-programmer' },
    { id: 'format', label: '格式化', desc: 'Markdown 摘要', icon: FileText, x: 220, y: 140, color: 'text-clone' },
    { id: 'output', label: '推送', desc: '发到飞书', icon: Send, x: 380, y: 140, color: 'text-success' },
  ]
        ...
        <div className='flex-1 flex'>
          <div className='flex-1 relative bg-surface-2/50 overflow-hidden'>
            ...
            <svg className='absolute inset-0 w-full h-full pointer-events-none'>
              <line x1='140' y1='66' x2='220' y2='66' stroke='var(--color-border-hover)' strokeWidth='2' strokeDasharray='4,3' />
              <line x1='300' y1='66' x2='380' y2='66' stroke='var(--color-border-hover)' strokeWidth='2' strokeDasharray='4,3' />
              <line x1='300' y1='66' x2='300' y2='166' stroke='var(--color-border-hover)' strokeWidth='2' strokeDasharray='4,3' />
              <line x1='300' y1='166' x2='380' y2='166' stroke='var(--color-border-hover)' strokeWidth='2' strokeDasharray='4,3' />
            </svg>
            {NODES.map(n => (
              <div
                key={n.id}
              ...
              <div>
                ...
                <div className='space-y-1'>
                  {['github_api', 'markdown_write', 'feishu_send'].map(t => (
                    <div key={t} className='flex items-center gap-1.5 px-2 py-1 bg-surface-3/60 rounded text-[10px] font-mono text-text-secondary'>
...
function SkillRow({ skill, onSelect }: { skill: Skill; onSelect: (s: Skill) => void }) {
  return (
    ...
    >
      ...
      {skill.installed ? (
        <button className='p-1 text-text-muted hover:text-text-secondary transition-colors' onClick={e => e.stopPropagation()}>
          <Pencil size={14} />
        </button>
      ) : (
        <button
    ...
    <div>
      ...
      <div className='grid grid-cols-3 gap-3 mb-8'>
        {FEATURED_SKILLS.map(f => (
          <div key={f.title} className={`${f.color} rounded-xl p-5 cursor-pointer hover:opacity-90 transition-opacity`}>
            <div className='text-[10px] font-medium text-text-muted uppercase tracking-wider mb-2'>{f.badge}</div>
            <div className='text-[14px] font-semibold text-text-primary leading-snug mb-1'>{f.title}</div>
    ...
    <div>
      ...
      <div className='flex items-center gap-2 mb-5'>
        {['All', 'integration', 'developer', 'productivity', 'design', 'business'].map(cat => (
          <button key={cat} className={`px-3 py-1.5 rounded-lg text-[12px] transition-colors ${
            cat === 'All' ? 'bg-accent text-accent-fg font-medium' : 'text-text-secondary hover:bg-surface-3'
          }`}>
            {cat === 'All' ? cat : `#${cat}`}
          </button>
        ))}
    ...
    <div className='h-full overflow-y-auto'>
      <div className='max-w-3xl mx-auto px-6 py-8'>
        ...
        <div className='flex items-center justify-between mb-6'>
          ...
          <div className='flex items-center gap-2'>
            <button className='p-1.5 rounded-lg text-text-muted hover:text-text-secondary hover:bg-surface-3 transition-colors' title='Refresh'>
              <RefreshCw size={13} />
            </button>

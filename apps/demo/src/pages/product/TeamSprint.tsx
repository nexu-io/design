...
import {
  List, GanttChart, ChevronRight, AlertTriangle,
  Users, Calendar, ArrowRight,
} from 'lucide-react'
import {
  TEAM_MEMBERS, SPRINT_TASKS, SPRINT_TASKS_GANTT, SPRINT_META,
  type SprintTask,
} from './teamData'
...
function ListView({ selectedTask, onSelectTask }: {
  selectedTask: SprintTask | null; onSelectTask: (t: SprintTask) => void
}) {
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-2 p-2.5 bg-surface-1 border rounded-lg cursor-pointer transition-colors ${
                    isSelected ? 'ring-2 ring-accent/30 border-accent/40' : 'border-border hover:border-border-hover'
                  }`}
                >
                  ...
                  <span className='text-[10px] text-text-muted tabular-nums w-8 text-right'>{task.progress}%</span>
...
function GanttView({ selectedTask, onSelectTask }: {
  selectedTask: SprintTask | null; onSelectTask: (t: SprintTask) => void
}) {
      ...
      <div className='min-w-[700px]'>
        ...
        <div className='flex mb-1'>
          ...
          <div className='flex-1 flex'>
            {days.map(d => (
              ...
              >
                <div className={`text-[8px] ${d.isToday ? 'text-accent font-bold' : 'text-text-muted'}`}>
                  {d.weekday}
                </div>
                <div className={`text-[10px] ${d.isToday ? 'text-accent font-bold' : 'text-text-secondary'}`}>
                  {d.day}
                </div>
              </div>
            ))}
        ...
        <div className='flex relative'>
          ...
          <div className='flex-1 relative h-0'>
            <div
              ...
              style={{ left: `${((todayOffset + 0.5) / totalDays) * 100}%`, height: `${SPRINT_TASKS_GANTT.length * 36 + 8}px` }}
            />
          </div>
        </div>
        ...
        <div className='space-y-1'>
          {SPRINT_TASKS_GANTT.map(task => {
            ...
            const isSelected = selectedTask?.id === task.id
            const dep = task.dependency ? SPRINT_TASKS_GANTT.find(t => t.id === task.dependency) : null
            ...
            return (
              ...
              >
                ...
                <div className='w-[180px] shrink-0 flex items-center gap-1.5 px-2'>
                  <span className={`text-[8px] px-1 py-0.5 rounded font-bold ${style.bg} ${style.text}`}>
                    {task.id}
                  </span>
                  <span className='text-[11px] text-text-primary truncate flex-1'>{task.title}</span>
                  {task.status === 'blocked' && <AlertTriangle size={10} className='text-danger shrink-0' />}
                </div>
                ...
                <div className='flex-1 relative h-full'>
                  {/* Dependency arrow */}
                  {dep && (
                    <div className='absolute top-1/2 -translate-y-1/2 z-[1]'
        ...
        <div className='flex items-center gap-4 mt-4 px-2'>
          {Object.entries(STATUS_STYLES).map(([key, st]) => (
            <div key={key} className='flex items-center gap-1'>
              <div className={`w-3 h-2 rounded-sm ${STATUS_BAR_COLORS[key as SprintTask['status']]}`} />
...
export default function SprintTab({ selectedTask, onSelectTask }: {
  selectedTask: SprintTask | null; onSelectTask: (t: SprintTask) => void
}) {
        ...
        <div className='flex items-center gap-1 bg-surface-2 rounded-lg p-0.5'>
          <button
            ...
            className={`flex items-center gap-1 px-2.5 py-1 rounded-md text-[11px] font-medium transition-colors ${
              view === 'list' ? 'bg-surface-0 text-text-primary shadow-sm' : 'text-text-muted hover:text-text-secondary'
            }`}
          >
            <List size={12} /> 列表
          </button>
          <button
            ...
            className={`flex items-center gap-1 px-2.5 py-1 rounded-md text-[11px] font-medium transition-colors ${
              view === 'gantt' ? 'bg-surface-0 text-text-primary shadow-sm' : 'text-text-muted hover:text-text-secondary'
            }`}
          >
        ...
        <div className='flex items-center justify-between mt-1.5 text-[10px] text-text-muted'>
          <div className='flex items-center gap-3'>
            ...
            <span className='flex items-center gap-1'>
              <Users size={9} /> {TEAM_MEMBERS.filter(m => SPRINT_TASKS.some(t => t.assignee === m.name)).length} 人参与
            </span>
          </div>
          <span>剩余 7 天</span>
        </div>

  ...
  return (
    <div className="flex-1 flex flex-col items-center px-6 overflow-y-auto">
      <div className="w-full max-w-lg flex flex-col items-center py-6">
        ...
        <h2 className="text-base font-semibold text-text-primary mb-0.5">Good evening, Tom</h2>
        ...
        <div className="w-full p-2.5 rounded-lg bg-surface-2/60 border border-border/50 mb-3">
          ...
          <div className="flex flex-wrap gap-1">
            {[
              ...
              <span
                key={s.label}
                className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded border text-[9px] ${s.color}`}
              >
                {s.label}
        ...
        <div className="p-2 border-b border-border">
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className="p-1 rounded-md hover:bg-surface-3 text-text-secondary"
              title="新建对话"
            >
              <Plus size={13} />
            </button>
              ...
              >
                <div className="flex items-start gap-1.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div
                      className={`text-[11px] font-medium truncate ${isActive ? "" : "text-text-primary"}`}
                    >
                      {s.title}
                    </div>
                    <div className="flex items-center gap-1 mt-0.5">
                      <span
                        className={`text-[9px] ${isActive ? "text-accent-fg/70" : "text-text-muted"}`}
                      >
                        {s.time}
                      </span>
                      ...
                      {s.ops > 0 && (
                        <span
                          className={`text-[8px] font-mono ${isActive ? "text-accent-fg/70" : "text-text-muted"}`}
                        >
                          {s.ops} ops
                        </span>
                      )}
                  ...
                  {s.badge && !isActive && (
                    <span className="text-[7px] px-1 py-0.5 bg-clone/10 text-clone rounded shrink-0 mt-0.5">
                      {s.badge}
                    </span>
                  )}
                </div>
        ...
        <div className="flex-1 flex min-w-0">
          <div className="flex-1 flex flex-col min-w-0">
            <div className="h-10 border-b border-border flex items-center justify-between px-3">
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  className={`p-1 rounded transition-colors ${previewOpen ? "text-text-primary bg-surface-3" : "text-text-muted"}`}
                >
                  <Eye size={13} />
                </button>
                <button type="button" className="p-1 rounded hover:bg-surface-3 text-text-muted">
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-2.5">
              {MESSAGES.map((msg) => (
                <div
                  key={`${msg.from}-${msg.content ?? msg.cards?.[0]?.title ?? msg.tool?.name}`}
                ...
                >
                  ...
                  <div className="max-w-[80%] space-y-1">
                    {msg.cards && msg.cards.length > 0 && <ChatCardGroup cards={msg.cards} />}
                    {msg.tool && (
            ...
            <div className="border-t border-border p-2">
              <div className="flex items-center gap-1.5 bg-surface-2 border border-border rounded-xl px-3 py-1.5">
                ...
                <button type="button" className="p-1 bg-accent text-accent-fg rounded-lg">
                  <Send size={12} />
                </button>
              </div>
            </div>
                      ...
                      >
                        ...
                        {op.added && (
                          <div className="flex items-center gap-1.5 mt-0.5 pl-5 text-[8px]">
                            ...
                            {op.removed && (
                              <span className="text-danger font-mono">-{op.removed}</span>
                            )}

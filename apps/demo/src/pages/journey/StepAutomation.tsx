...
import {
  Clock, Brain, Zap, Bell, Calendar, FileText, Users,
  CheckCircle, Shield, Search, Sparkles, BarChart3,
  Mail, TrendingUp, Pause, MoreHorizontal,
...
const TASKS = [
  { name: '今日战况复盘', cron: '每天 22:00', icon: Brain, enabled: true, lastRun: '今天 22:00', stats: '扫荡 7,660 条 · 评论 21 次' },
  { name: '早间日报', cron: '每天 08:30', icon: Calendar, enabled: true, lastRun: '今天 08:30' },
  { name: 'Action Item 检查', cron: '每天 3 次', icon: CheckCircle, enabled: true, lastRun: '今天 15:00', stats: '检查 7 项 · 已完成 3' },
...
const SKILLS = [
  { name: 'Memory & Notes', desc: '自动记忆管理，知识沉淀', icon: FileText, installed: true, installs: '12.4k' },
  { name: 'Task Manager', desc: '待办创建、追踪、提醒', icon: BarChart3, installed: true, installs: '11.2k' },
  { name: 'Web Research', desc: '联网搜索 + 信息整理', icon: Search, installed: true, installs: '10.1k' },
  { name: 'Code Automation', desc: '代码生成、PR、自动化', icon: Code, installed: true, installs: '8.3k' },
  { name: 'Daily Digest', desc: '每日日程 + 待办汇总', icon: FileText, installed: true, installs: '9.8k' },
]

const RECOMMENDED = [
  { name: 'Contact Intelligence', desc: '联系人健康评分 + 关系管理', icon: Users, installs: '7.2k' },
      ...
      <div className='flex items-center gap-3 mb-6'>
        <button
          ...
          className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-[13px] font-medium transition-colors ${
            view === 'automation' ? 'bg-accent text-accent-fg' : 'bg-surface-2 border border-border text-text-secondary hover:bg-surface-3'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-[13px] font-medium transition-colors ${
            view === 'skills' ? 'bg-accent text-accent-fg' : 'bg-surface-2 border border-border text-text-secondary hover:bg-surface-3'
          }`}
        >
        ...
        <div className='space-y-6'>
          ...
          <div>
            ...
            <div className='grid grid-cols-3 gap-2'>
              {TEMPLATES.map(t => (
                <button key={t.name} className='p-2.5 bg-surface-2 border border-border rounded-xl hover:border-border-hover transition-colors text-left'>
                  <div className={`w-6 h-6 rounded-lg ${t.color.split(' ')[0]} flex items-center justify-center mb-1.5`}>
          ...
          <div>
            <div className='flex items-center gap-2 mb-2'>
              ...
              <span className='text-[9px] text-text-muted bg-surface-3 px-1.5 py-0.5 rounded-full'>{TASKS.length}</span>
            </div>
            <div className='space-y-2'>
              {TASKS.map(task => (
                <div key={task.name} className='flex items-start gap-3 p-3 bg-surface-2 border border-border rounded-xl hover:border-border-hover transition-colors'>
                  ...
                  <div className='flex-1 min-w-0'>
                    <div className='flex items-center gap-2'>
                      ...
                      <span className='text-[9px] px-1 py-0.5 bg-surface-3 rounded text-text-muted font-mono'>{task.cron}</span>
                    </div>
                    <div className='text-[10px] text-text-muted mt-0.5'>上次运行：{task.lastRun}</div>
                    {task.stats && <div className='mt-1 px-2 py-1 bg-surface-3 rounded text-[10px] text-text-secondary'>{task.stats}</div>}
                  </div>
                  <div className='flex items-center gap-1 shrink-0'>
                    <button className='p-1 rounded-md hover:bg-surface-3 text-success'><Pause size={12} /></button>
                    <button className='p-1 rounded-md hover:bg-surface-3 text-text-muted'><MoreHorizontal size={12} /></button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            <div className='space-y-1.5'>
              {PROACTIVE_RULES.map(rule => (
                <div key={rule.name} className='flex items-center gap-3 p-2.5 bg-surface-2 border border-border rounded-lg'>
                  ...
                  <div className='flex-1 min-w-0'>
                    ...
                    <div className='text-[10px] text-text-tertiary italic mt-0.5'>{rule.example}</div>
                  </div>
          ...
          <div>
            <div className='flex items-center gap-2 mb-2'>
              ...
              <span className='text-[12px] font-medium text-text-secondary'>Session 中的卡片产出</span>
              <span className='text-[9px] text-text-muted bg-surface-3 px-1.5 py-0.5 rounded-full'>预览</span>
            </div>
            <div className='bg-surface-1 border border-border rounded-xl p-3'>
              <div className='flex items-center gap-2 mb-2'>
                <div className='w-5 h-5 rounded-full bg-clone/10 flex items-center justify-center text-[10px]'>😊</div>
                <span className='text-[10px] font-medium text-text-primary'>分身执行自动化后产出卡片 →</span>
              </div>
        ...
        <div className='space-y-6'>
          ...
          <div>
            ...
            <div className='flex items-center gap-3 mb-4'>
              <span className='text-[12px] font-medium text-text-primary border-b-2 border-text-primary pb-1'>Installed <span className='text-text-muted ml-1'>{SKILLS.length}</span></span>
              <span className='text-[12px] text-text-secondary pb-1'>Featured</span>
              <span className='text-[12px] text-text-secondary pb-1'>Explore <span className='text-text-muted ml-1'>12</span></span>
            </div>

            <div className='space-y-1'>
              {SKILLS.map(s => (
                <div key={s.name} className='flex items-center gap-3 p-2.5 hover:bg-surface-3/50 rounded-lg transition-colors'>
          ...
          <div>
            <div className='flex items-center gap-2 mb-2'>
              ...
              <span className='text-[12px] font-medium text-text-secondary'>Recommended for you</span>
            </div>
            <div className='grid grid-cols-2 gap-2'>
              {RECOMMENDED.map(s => (
                <div key={s.name} className='flex items-center gap-2.5 p-3 bg-surface-2 border border-border rounded-xl hover:border-border-hover transition-colors cursor-pointer'>
                  <div className='w-8 h-8 rounded-lg bg-surface-3 flex items-center justify-center shrink-0'>
                    <s.icon size={14} className='text-text-secondary' />
                  </div>
                  <div className='flex-1 min-w-0'>
                  ...
                  <button className='p-1 rounded-md hover:bg-surface-3 text-text-muted'><Plus size={13} /></button>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            <div className='flex items-center gap-2 mb-2'>
              <MessageSquare size={12} className='text-text-muted' />
              <span className='text-[12px] font-medium text-text-secondary'>Session 中的技能卡片</span>
              <span className='text-[9px] text-text-muted bg-surface-3 px-1.5 py-0.5 rounded-full'>预览</span>
            </div>
            <div className='bg-surface-1 border border-border rounded-xl p-3'>

...
import {
  ...
  Shield, Globe, Database, Link2, Sparkles, Brain,
        ...
        {shown.map(item => (
          ...
          >
            <div className='flex items-center gap-2 mb-1'>
              ...
              {item.status === 'connected' && <CheckCircle size={11} className='text-success ml-auto' />}
              {item.status === 'coming' && <span className='text-[9px] text-text-muted ml-auto'>即将</span>}
            </div>
            <div className='text-[10px] text-text-muted'>{item.desc}</div>
            {item.status === 'connected' && (
              <div className='text-[10px] text-success mt-1 flex items-center gap-1'>
                <RefreshCw size={9} /> {item.importable}
              </div>
            )}
      ...
      <div className='flex flex-wrap gap-1.5'>
        {skills.map(s => (
          ...
          >
            <span>{s.icon}</span>
            {s.name}
            {s.recommended && !s.selected && <span className='text-[9px] text-clone'>推荐</span>}
      ...
      <div className='p-3 bg-clone/5 border border-clone/10 rounded-xl'>
        ...
        <div className='mt-2 space-y-1'>
          {[
            ...
            <div key={item} className='text-[11px] text-text-secondary'>
              {item}
            </div>
          ))}
        </div>
...
export default function StepOnboarding() {
  ...
  const [activeWidget, setActiveWidget] = useState<ActiveWidget>('integrations')

  return (
    <div className='max-w-2xl mx-auto py-8 px-6'>
      <div className='bg-surface-0 border border-border/50 rounded-2xl shadow-[var(--shadow-dropdown)] overflow-hidden flex flex-col' style={{ maxHeight: '680px' }}>
        {/* Header */}
        <div className='px-5 py-3 border-b border-border flex items-center gap-3 shrink-0'>
          <div className='w-8 h-8 rounded-full bg-clone/15 flex items-center justify-center text-lg'>😊</div>
            ...
            {STEPS.map((step, i) => (
              <div key={step.id} className='flex items-center gap-1 shrink-0'>
                <div className={`flex items-center gap-1 px-2 py-1 rounded-md text-[10px] transition-colors ${
                  step.status === 'done' ? 'bg-success-subtle text-success'
                    : step.status === 'active' ? 'bg-accent text-accent-fg font-medium'
                      : 'bg-surface-3 text-text-muted'
                }`}>
                  {step.status === 'done' ? <Check size={9} /> : <step.icon size={9} />}
                  {step.label}
                </div>
                {i < STEPS.length - 1 && <ChevronRight size={9} className='text-text-muted shrink-0' />}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className='flex-1 overflow-y-auto p-4 space-y-2.5 min-h-0'>
            ...
            <div key={i}>
              {msg.from === 'system' ? (
                <div className='flex items-center gap-2 py-1.5'>
                  ...
                  <span className='text-[9px] text-text-muted shrink-0'>{msg.content}</span>
                  <div className='h-px flex-1 bg-border' />
                </div>
              ) : msg.from === 'user' ? (
                ...
                <div className='flex gap-2.5'>
                  <div className='w-6 h-6 rounded-full bg-clone/15 flex items-center justify-center text-[11px] shrink-0 mt-0.5'>😊</div>
                  <div className='max-w-[85%]'>
                    ...
                    {msg.widget === 'integrations' && activeWidget === 'integrations' && <IntegrationsWidget />}
                    {msg.widget === 'integrations' && activeWidget === 'skills' && <SkillsWidget />}
                    {msg.widget === 'integrations' && activeWidget === 'upload' && <UploadWidget />}
                    {msg.widget === 'integrations' && activeWidget === 'team' && <TeamSetupWidget />}
                    {msg.widget === 'integrations' && activeWidget === 'progress' && <ProgressWidget />}
                  </div>
                </div>
              )}
            </div>
          ...
          {/* Memory cards */}
          <div className='ml-8'>
            <ChatCardGroup cards={ONBOARDING_CARDS} />
          </div>
          ...
          <div className='flex items-center gap-2 pt-2 flex-wrap'>
            ...
            {([
              ...
              <button
                key={action.label}
                onClick={() => setActiveWidget(activeWidget === action.widget ? null : action.widget)}
                className={`flex items-center gap-1 px-2 py-1 rounded-md border text-[10px] transition-colors ${
                  activeWidget === action.widget
                    ? 'bg-accent/10 border-accent/30 text-accent font-medium'
                    : 'bg-surface-2 border-border text-text-secondary hover:text-text-primary hover:border-border-hover'
                }`}
              >
                <action.icon size={10} />
                {action.label}
        ...
        <div className='border-t border-border p-3 shrink-0'>
          <div className='flex items-end gap-2 bg-surface-1 border border-border rounded-xl px-3.5 py-2.5'>
            ...
            <div className='flex items-center gap-1.5 shrink-0'>
              ...
              <button className='p-1.5 bg-accent text-accent-fg rounded-lg hover:bg-accent-hover transition-colors'>
                <Send size={12} />
              </button>
            </div>
          </div>

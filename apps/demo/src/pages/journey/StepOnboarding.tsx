          ...
          >
            ...
            {item.status === "available" && (
              <Button
                variant="ghost"
                size="xs"
                className="mt-1.5 h-auto w-full gap-1 bg-accent/10 px-2 py-1 text-[10px] font-medium text-accent hover:bg-accent/15"
              >
                <Link2 size={9} /> 授权连接
              </Button>
            )}
    ...
    <div className="mt-2 p-4 border-2 border-dashed border-border rounded-xl bg-surface-1 text-center hover:border-accent/40 transition-colors cursor-pointer">
      ...
      <div className="flex items-center justify-center gap-2 mt-3">
        {["简历", "笔记", "项目文档", "会议录音"].map((tag) => (
          <span
            key={tag}
            className="text-[10px] px-2 py-0.5 bg-surface-3 rounded-md text-text-muted"
          >
            {tag}
          </span>
        ))}
    ...
    <div className="mt-2 space-y-3">
      <div className="p-3 bg-surface-1 border border-border rounded-xl">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-[11px] text-clone font-medium">
        ...
        <Button
          variant="ghost"
          size="sm"
          className="mt-2.5 w-full gap-1.5 bg-accent/10 text-[11px] font-medium text-accent hover:bg-accent/15"
        >
...
export default function StepOnboarding() {
  ...
  return (
    <div className="max-w-2xl mx-auto py-8 px-6">
      <div
        className="bg-surface-0 border border-border/50 rounded-2xl shadow-[var(--shadow-dropdown)] overflow-hidden flex flex-col"
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2.5 min-h-0">
          {MESSAGES.map((msg) => (
            <div key={`${msg.from}-${msg.content.slice(0, 20)}`}>
              {msg.from === "system" ? (
          ...
          <div className="flex items-center gap-2 pt-2 flex-wrap">
            ...
            {[
              ...
              <button
                ...
                key={action.label}
                onClick={() =>
                  setActiveWidget(activeWidget === action.widget ? null : action.widget)
                }
        ...
        <div className="border-t border-border p-3 shrink-0">
          ...
          <div className="flex items-center justify-between mt-2 px-1">
            ...
            <Button
              variant="ghost"
              size="xs"
              className="h-auto gap-1 px-0 text-[10px] text-text-muted hover:text-accent"
            >
              跳过初始化，直接使用
              <ArrowRight size={9} />
            </Button>

import {
  Users, User, MessageSquare, UserPlus, Radio, GitPullRequest,
...
const SIMPLE_TASKS = [
  { id: 'TK-001', title: 'Gateway 重构', status: 'in_progress', priority: 'high', assignee: '李四', executor: 'agent' },
  { id: 'TK-002', title: '前端集成接入', status: 'todo', priority: 'urgent', assignee: '张三', executor: 'hybrid' },
  { id: 'TK-003', title: '暗色主题设计稿', status: 'done', priority: 'medium', assignee: '王五', executor: 'human' },
]
...
function StatsBar() {
  ...
  const stats = [
    { label: 'OKR 完成率', value: '86%', color: 'text-accent' },
    { label: '分身在线', value: `${online}/${TEAM_MEMBERS.length}`, color: 'text-success' },
    { label: '今日卡片', value: '5', color: 'text-clone' },
    { label: '待对齐', value: '1', color: 'text-warning' },
    { label: '活跃任务', value: '4', color: 'text-clone' },
    { label: 'Sprint 进度', value: '58%', color: 'text-info' },
  ]
  return (
  ...
  return (
    <div className='border border-border rounded-xl bg-surface-1 px-4 py-3 border-l-2 border-l-clone/50'>
      ...
      <div className='flex items-center gap-3 text-[10px] text-text-muted'>
        ...
        <span className={executor.color}>{task.executor === 'agent' ? 'Agent' : task.executor === 'hybrid' ? '人+Agent' : '人工'}</span>
      </div>
    </div>
  )
}
    ...
    <div className='max-w-6xl mx-auto p-6 space-y-6'>
      <div className='text-center mb-6'>
        ...
        <p className='text-[14px] text-text-secondary max-w-xl mx-auto'>
          当团队成员都有分身后，分身之间可以代替人交流、自动对齐、检测依赖风险。
          所有交互通过 IM 卡片推送到飞书/Slack 群。
        </p>
      </div>
              ...
              {TEAM_MEMBERS.map(m => (
                <div key={m.name} className='flex items-center gap-3 p-2.5 bg-surface-0 rounded-lg'>
                  <div className='relative'>
                    ...
                    <div className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-surface-1 ${STATUS_COLOR[m.status]}`} />
                  </div>
                  <div className='flex-1 min-w-0'>
                    <div className='flex items-center gap-2'>
                      <span className='text-[12px] font-medium text-text-primary'>{m.name}</span>
                      <span className='text-[9px] px-1 py-0.5 bg-clone/10 text-clone rounded'>Lv.{m.level}</span>
                    </div>
              ...
              <div>
                ...
                <div className='flex items-center gap-2 mb-3'>
                  ...
                  <span className='text-[11px] font-semibold text-text-primary'>
                    {cardTab === 'standup' ? '赵六的分身' : cardTab === 'proxy' ? '你的分身' : '张三的分身'}
                  </span>
      ...
      <div className='border border-border rounded-xl bg-surface-1 p-5'>
        ...
        <div className='grid grid-cols-3 gap-4'>
          {[
            { icon: '🤝', title: '分身代问', desc: '你的分身向其他成员的分身查询任务进度，自动回复，无需打扰本人。结果以 IM 卡片呈现。', tag: 'P0 · Phase 4' },
            { icon: '📊', title: '站会自动化', desc: '每日自动汇总团队分身任务进度，生成站会 IM 卡片推送到群，包含风险提醒。', tag: 'P0 · Phase 4' },
            { icon: '⚠️', title: '依赖感知与主动对齐', desc: '检测跨成员任务依赖风险，自动发起对齐请求卡片，一键确认调整排期。', tag: 'P1 · Phase 5' },
          ].map(s => (
            <div key={s.title} className='p-4 bg-surface-0 border border-border rounded-xl'>
              ...
              <span className='text-[9px] px-2 py-0.5 bg-clone/10 text-clone rounded-full font-medium'>{s.tag}</span>
            </div>
          ))}
        </div>
      </div>

...
export default function SlackDemo({ scenarioKey, className, height }: { scenarioKey: string; className?: string; height?: string }) {
  const scenario = DEMO_SCENARIOS.find((s) => s.key === scenarioKey) ?? DEMO_SCENARIOS[0];
  const activeChannel = scenario.channel.replace('#', '');

  return (
    <div className={`overflow-hidden rounded-xl shadow-2xl border border-[#ccc]/30 ${className ?? ''}`}>
      {/* Title bar */}
      ...
      <div className={`flex ${height ?? 'h-[400px] sm:h-[520px]'}`}>
        ...
        <div className='hidden sm:flex w-[120px] shrink-0 bg-[#3f0e40] flex-col py-2 px-1 border-r border-[#5c2d5e] overflow-hidden'>
          ...
          <div className='space-y-px'>
            {SLACK_DMS.map((dm) => (
              <div key={dm.name} className='flex gap-1 items-center px-1.5 py-[2px] text-[10px] text-white/60 truncate'>
                <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${dm.online ? 'bg-[#2bac76]' : 'border border-white/30'}`} />

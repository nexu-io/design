...
function ColorLayer({ shape, staggerDelay, trigger }: {
  ...
  return (
    <g ref={gRef} transform={`translate(${shape.cx},${shape.cy}) scale(0) translate(${-shape.cx},${-shape.cy})`}>
...
export function NexuLoadingScreen({ show }: { show: boolean }) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-white">
      <NexuLoader size={96} />
      <p className="mt-5 text-sm text-[#999] tracking-wide">
        Loading...
      </p>
    </div>
  );

...
export default function LanguageSwitcher({ variant = 'light', size = 'sm' }: Props) {
  ...
  const base = size === 'xs' ? 'h-8 min-w-[130px] px-3 text-[12px]' : size === 'sm' ? 'h-11 min-w-[162px] px-3.5 text-[13px]' : 'h-12 min-w-[176px] px-4 text-[14px]';
  const optionBase = size === 'xs' ? 'h-8 px-3 text-[12px]' : size === 'sm' ? 'h-10 px-4 text-[13px]' : 'h-11 px-4 text-[14px]';
  ...
  const colors = {
    light: {
      trigger: 'border border-dark-border bg-dark-surface/92 text-white shadow-dropdown-dark backdrop-blur-md hover:bg-dark-surface-hover',
    ...
    dark: {
      trigger: 'border border-border bg-white/92 text-text-primary shadow-dropdown backdrop-blur-md hover:bg-white',
  ...
  return (
    <div ref={rootRef} className="relative inline-flex">
      ...
      >
        ...
        <ChevronDown size={size === 'xs' ? 13 : size === 'sm' ? 16 : 17} className={`shrink-0 transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>

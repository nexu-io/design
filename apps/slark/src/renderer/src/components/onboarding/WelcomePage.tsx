  ...
  const renderEmailFlow = (): React.ReactElement => {
    if (emailStep === 'email') {
      return (
        <>
          ...
          <button onClick={resetEmail} className="text-sm text-muted-foreground hover:text-foreground transition-colors">
            <span className="flex items-center gap-1 justify-center"><ArrowLeft className="h-3.5 w-3.5" /> {t('common.back')}</span>
          </button>
        </>
      )
    }
    ...
    if (emailStep === 'verify') {
      return (
        <>
          ...
          >
            <span className="flex items-center gap-1 justify-center"><ArrowLeft className="h-3.5 w-3.5" /> {t('onboarding.changeEmail')}</span>
          </button>
        </>
      )
    }
    ...
    return (
      <>
        <div className="text-center">
          <p className="text-sm text-muted-foreground">
            {t('onboarding.setPasswordFor')} <span className="font-medium text-foreground">{email}</span>
          </p>
        </div>
        ...
        <button
          onClick={() => { setEmailStep('verify'); setPassword(''); setConfirmPassword(''); setError('') }}
          className="text-sm text-muted-foreground hover:text-foreground transition-colors"
        >
          <span className="flex items-center gap-1 justify-center"><ArrowLeft className="h-3.5 w-3.5" /> {t('common.back')}</span>
        </button>

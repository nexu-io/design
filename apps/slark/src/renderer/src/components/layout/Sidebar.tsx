...
import { Building2, Palette, User, ChevronDown, UserPlus, Settings as SettingsIcon, LogOut } from 'lucide-react'
            ...
            <div className="absolute top-full left-0 z-50 mt-1 w-72 rounded-xl border border-border bg-popover text-foreground shadow-2xl overflow-hidden">
              <div className="flex items-center gap-3 px-3 py-3 border-b border-border">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-sm font-semibold truncate">{workspace?.name ?? 'Nexu'}</div>
                  <div className="text-[11px] text-muted-foreground truncate">{workspaceHandle}</div>
                </div>
              </div>
...
function SettingsSidebar(): React.ReactElement {
  ...
  const items: { path: string; labelKey: TranslationKey; icon: typeof Building2; exact: boolean }[] = [
    { path: '/settings', labelKey: 'settings.workspace', icon: Building2, exact: true },
    { path: '/settings/appearance', labelKey: 'settings.appearance', icon: Palette, exact: false },
    { path: '/settings/profile', labelKey: 'settings.profile', icon: User, exact: false }
  ]

  return (
    <div className="flex-1 overflow-y-auto px-2 space-y-0.5">
      {items.map(({ path, labelKey, icon: Icon, exact }) => {

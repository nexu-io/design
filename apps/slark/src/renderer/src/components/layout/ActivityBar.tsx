        ...
        {menuOpen && (
          ...
          <div className="absolute top-0 left-[60px] z-50 w-[320px] rounded-2xl border border-border bg-popover text-popover-foreground shadow-2xl overflow-hidden py-1.5">
            {workspaces.map((ws, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {idx < 9 && (
                    <span className="text-xs text-muted-foreground shrink-0 tracking-wider">⌘{idx + 1}</span>
                  )}
                </button>
              )
            })}
            ...
            >
              ...
              <span className="flex-1 text-left text-sm text-foreground">{t('workspace.addWorkspace')}</span>
              {workspaces.length >= 5 && (
                <span className="text-[10px] text-muted-foreground shrink-0">{workspaces.length}/5</span>
              )}
            </button>

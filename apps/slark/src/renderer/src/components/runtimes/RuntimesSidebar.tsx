...
const MOCK_DETECTION_POOL: DetectedRuntime[] = [
  { type: 'claude-code', name: 'Claude Code', desc: 'Anthropic',   version: '1.0.12', path: '/usr/local/bin/claude' },
  { type: 'opencode',    name: 'OpenCode',    desc: 'Open-source', version: '0.5.3',  path: '/usr/local/bin/opencode' },
  { type: 'gemini-cli',  name: 'Gemini CLI',  desc: 'Google',      version: '0.1.0',  path: '/usr/local/bin/gemini' },
...
export function RuntimesSidebar(): React.ReactElement {
  ...
  const filtered = tab === 'mine'
    ? runtimes.filter((r) => r.ownerId === currentUserId)
    : runtimes
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-3 pb-2 flex items-center gap-2">
        <span className="text-xs text-nav-muted">{t('runtimes.online', { online: String(onlineCount), total: String(runtimes.length) })}</span>
      </div>
          ...
          <div className="mt-3 pt-3 border-t border-nav-border">
            <div className="flex items-center justify-between px-2 pb-1.5">
              <div className="flex items-center gap-1.5">
                ...
                {scanning && (
                  <RefreshCw className="h-3 w-3 text-nav-muted animate-spin" />
                )}
              </div>
              {!scanning && (
                <button
            ...
            {!scanning && detectedNotAdded.length === 0 ? (
              <p className="px-2 py-2 text-xs text-nav-muted/70">
                {t('runtimes.noNew')}
              </p>
            ) : (

...
export function MessageInput({ channelId, isDmWithAgent, channel }: MessageInputProps): React.ReactElement {
  const t = useT()
  ...
  return (
    <div className="relative px-4 pb-4">
      ...
      <div
        className={cn(
          'message-input-container flex items-center gap-1 rounded-md border px-2 py-1 transition-all duration-200 ease-out',
          isFocused
            ? 'border-ring/70 bg-background'
      ...
      >
        <textarea
          ...
          placeholder={channel.type === 'dm' ? t('chat.messagePerson', { name: channel.name }) : t('chat.messageChannel')}
          rows={1}
          className="message-textarea flex-1 resize-none bg-transparent text-sm placeholder:text-muted-foreground focus:outline-none px-1.5 py-1.5"
        />
        <div className="flex items-center gap-0.5 shrink-0">
          ...
          <button
            ...
            disabled={!hasText}
            className={cn(
              'message-send-btn flex h-7 w-7 items-center justify-center rounded-md transition-colors',
              hasText
                ? 'text-nexu-primary hover:bg-accent'

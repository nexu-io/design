...
export function AddMembersDialog({ open, onOpenChange, channel }: AddMembersDialogProps): React.ReactElement | null {
  const t = useT()
  ...
  const filteredUsers = q
    ? availableUsers.filter((u) => u.name.toLowerCase().includes(q) || u.email.toLowerCase().includes(q))
    : availableUsers
  const filteredAgents = q
    ? availableAgents.filter((a) => a.name.toLowerCase().includes(q) || (a.description ?? '').toLowerCase().includes(q))
    : availableAgents
    ...
    >
      <div className="w-[440px] rounded-xl border border-border bg-background text-foreground shadow-xl overflow-hidden">
        <div className="flex items-center justify-between px-5 pt-5 pb-1">
          <h2 className="text-base font-semibold text-foreground">{t('addMembers.title', { name: channel.name })}</h2>
          <button
                ...
                <div className="space-y-0.5">
                  {filteredUsers.map((u) => {
                    ...
                    return (
                      ...
                      >
                        <img src={u.avatar} alt={u.name} className="h-7 w-7 rounded-full bg-secondary shrink-0" />
                        ...
                        <div className={cn(
                          ...
                          selected ? 'bg-foreground border-foreground text-background' : 'border-input'
                        )}>
                ...
                <div className="space-y-0.5">
                  {filteredAgents.map((a) => {
                    ...
                    return (
                      ...
                      >
                        {a.avatar ? (
                          <img src={a.avatar} alt={a.name} className="h-7 w-7 rounded-md bg-secondary shrink-0" />
                        ) : (
        ...
        <div className="flex items-center justify-between gap-2 px-5 pb-5 pt-1">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {totalSelected > 0 ? t('addMembers.addWithCount', { count: String(totalSelected) }) : t('common.add')}
            </button>
          </div>
        </div>
      </div>

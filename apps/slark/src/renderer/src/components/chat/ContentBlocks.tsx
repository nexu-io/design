...
function ImageBlock({ block, onExpand }: { block: Extract<ContentBlock, { type: 'image' }>; onExpand?: () => void }): React.ReactElement {
  return (
...
function FileBlock({ block }: { block: Extract<ContentBlock, { type: 'file' }> }): React.ReactElement {
...
function CodeBlock({ block, isMe, onExpand }: { block: Extract<ContentBlock, { type: 'code' }>; isMe: boolean; onExpand?: () => void }): React.ReactElement {
...
function ActionCard({ block }: { block: Extract<ContentBlock, { type: 'action' }> }): React.ReactElement {
  return (
    <div className={cn(
      ...
      <div className="min-w-0 flex-1">
        ...
        {block.description && (
          <p className="text-[12px] text-muted-foreground/70 mt-0.5 leading-relaxed">{block.description}</p>
        )}
        {block.tool && (
...
function ToolResultBlock({ block }: { block: Extract<ContentBlock, { type: 'tool-result' }> }): React.ReactElement {
        ...
        <div className="bg-[#0d1117] border-t border-white/[0.04]">
          {block.input && (
            <div className="px-3.5 pt-3 pb-2">
              <p className="text-[10px] uppercase tracking-widest text-white/20 mb-1.5 font-medium">Input</p>
              <pre className="text-[11px] font-mono text-white/50 whitespace-pre-wrap break-words leading-relaxed">{block.input}</pre>
            </div>
          )}
          <div className={cn('px-3.5 pb-3', block.input ? 'pt-1 border-t border-white/[0.04]' : 'pt-3')}>
            <p className="text-[10px] uppercase tracking-widest text-white/20 mb-1.5 font-medium">Output</p>
            <pre className="text-[11px] font-mono text-white/70 whitespace-pre-wrap break-words leading-relaxed">{block.output}</pre>
          </div>
        </div>
      )}
    </div>
...
function DiffBlock({ block, onExpand }: { block: Extract<ContentBlock, { type: 'diff' }>; onExpand?: () => void }): React.ReactElement {
  ...
  return (
    <div
      className={cn('w-[420px] rounded-xl overflow-hidden border border-white/[0.06]', onExpand && 'cursor-pointer')}
      onClick={onExpand}
    >
    ...
    <div className={cn(
      ...
      <div className="flex items-start gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          {block.description && (
            <p className="text-[12px] text-muted-foreground/60 mt-1 leading-relaxed">{block.description}</p>
          )}
        </div>
      </div>
...
function ProgressBlock({ block }: { block: Extract<ContentBlock, { type: 'progress' }> }): React.ReactElement {
...
export function ContentBlockRenderer({ block, isMe, onApprovalAction, onExpand }: ContentBlockRendererProps): React.ReactElement {
  const handleExpand = (): void => onExpand?.(block)

  switch (block.type) {
    case 'image': return <ImageBlock block={block} onExpand={handleExpand} />
    case 'file': return <FileBlock block={block} />
    case 'code': return <CodeBlock block={block} isMe={isMe} onExpand={handleExpand} />
    case 'action': return <ActionCard block={block} />
    case 'tool-result': return <ToolResultBlock block={block} />
    case 'diff': return <DiffBlock block={block} onExpand={handleExpand} />
    case 'approval': return <ApprovalBlock block={block} onAction={onApprovalAction} />

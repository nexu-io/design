...
export function ContentDetailOverlay({ block, onClose }: ContentDetailOverlayProps): React.ReactElement | null {
  useEffect(() => {
    if (!block) return
...
function ImageLightbox({ url, alt, onClose }: { url: string; alt?: string; onClose: () => void }): React.ReactElement {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={onClose}>
      ...
      <div className="max-w-[90vw] max-h-[85vh] p-4" onClick={(e) => e.stopPropagation()}>
        ...
        />
        {alt && (
          <p className="text-sm text-white/50 text-center mt-3">{alt}</p>
        )}
      </div>
    </div>
  )
      ...
      >
        <div className="flex items-center justify-between h-11 px-4 bg-white/[0.03] border-b border-white/[0.06] shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            ...
            {block.language && (
              <span className="text-[11px] text-white/25 font-mono bg-white/[0.05] px-2 py-0.5 rounded">{block.language}</span>
            )}
          </div>
          <div className="flex items-center gap-1 shrink-0 ml-3">
            <button
              ...
              className={cn(
                ...
                copied ? 'text-green-400' : 'text-white/30 hover:text-white/60 hover:bg-white/[0.05]'
              )}
            >

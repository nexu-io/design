...
function renderContent(content: string): React.ReactNode {
  ...
  return parts.map((part, i) => {
    if (part.startsWith('```') && part.endsWith('```')) {
      ...
      return (
        <pre key={i} className="my-2 rounded-lg bg-black/10 dark:bg-white/10 p-3 text-xs overflow-x-auto">
          <code>{code}</code>
        </pre>
      )
    }
...
function ChannelEmptyState({ channel }: { channel: Channel }): React.ReactElement {
  ...
  const memberAvatars = channel.members
    .map((m) => resolveRef(m))
  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
      {messages.map((msg, idx) => {
        ...
        const isSameSender =
          prevMsg &&
          prevMsg.sender.kind === msg.sender.kind &&
                ...
                <div className="flex flex-col gap-2 mt-1 max-w-full">
                  {msg.blocks.map((block, bi) => (
                    <ContentBlockRenderer
                      ...
                      block={block}
                      isMe={isMe}
                      onApprovalAction={(aid, action) => handleApproval(msg.id, msg.blocks, aid, action)}

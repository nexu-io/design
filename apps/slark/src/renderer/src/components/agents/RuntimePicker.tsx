        ...
        {selected ? (
          <>
            <div className={cn(
              ...
              selected.status === 'connected' && 'bg-nexu-online',
              selected.status === 'disconnected' && 'bg-nexu-offline',
              selected.status === 'error' && 'bg-destructive'
            )} />
            <span className="flex-1 truncate">{selected.name}</span>
          </>
        ) : (
          <span className="flex-1 text-muted-foreground">{t('runtime.noRuntime')}</span>
        )}
        ...
        <div className="absolute z-50 mt-1 w-full max-h-[240px] overflow-y-auto rounded-lg border border-border bg-popover shadow-lg animate-in fade-in-0 zoom-in-95 duration-100">
          {mockRuntimes.map((rt) => (
            <button
              type="button"
              key={rt.id}
              onClick={() => { onChange(rt.id); setOpen(false) }}
              className={cn(
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-xs text-muted-foreground">{rt.type}{rt.version ? ` v${rt.version}` : ''}</div>
              </div>

    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-3 text-[14px] text-text-secondary leading-relaxed max-w-2xl">
          nexu 支持两种模型接入方式：<strong>nexu Official</strong>（托管模型，登录即用）和{" "}
          <strong>BYOK</strong>（自带 API Key）。两种方式可随时切换，不影响已有对话和渠道连接。
        </p>
      </div>
      ...
      <Step n={1} title="打开 Settings">
        <p className="text-[13px] text-text-tertiary leading-relaxed">
          在 nexu 客户端左侧导航栏点击 <strong className="text-text-primary">Settings</strong>，进入
          AI Model Providers 配置页面。
        </p>
      ...
      <Step n={2} title="选择接入方式">
        <div className="space-y-6">
          ...
          <div className="p-4 rounded-xl border border-border bg-surface-1">
            ...
            <p className="text-[13px] text-text-tertiary leading-relaxed">
              在左侧供应商列表中选择 <strong className="text-text-primary">nexu Official</strong>
              ，点击 <strong className="text-text-primary">Sign in to nexu</strong>{" "}
              完成账号登录。登录后无需配置任何 API Key，Claude Sonnet 4.6、Claude Opus 4.6、Claude
              Haiku 4.5 等模型立即可用。
            </p>
          ...
          <div className="p-4 rounded-xl border border-border bg-surface-1">
            ...
            <p className="text-[13px] text-text-tertiary leading-relaxed mb-3">
              在左侧供应商列表中选择 <strong className="text-text-primary">Anthropic</strong>、
              <strong className="text-text-primary">OpenAI</strong>、
              <strong className="text-text-primary">Google AI</strong> 或其他供应商：
            </p>
            <ol className="space-y-1.5 text-[13px] text-text-tertiary">
              <li className="flex items-start gap-2">
                <span className="font-bold text-accent shrink-0">1.</span>在{" "}
                <strong className="text-text-primary">API Key</strong> 字段粘贴你的密钥。
              </li>
              <li className="flex items-start gap-2">
                <span className="font-bold text-accent shrink-0">2.</span>如需自定义代理，修改{" "}
                <strong className="text-text-primary">API Proxy URL</strong>。
              </li>
              <li className="flex items-start gap-2">
                <span className="font-bold text-accent shrink-0">3.</span>点击{" "}
                <strong className="text-text-primary">Save</strong>，nexu
                会自动验证密钥并加载可用模型列表。
              </li>
            </ol>
      ...
      <Step n={3} title="选择当前模型">
        <p className="text-[13px] text-text-tertiary leading-relaxed">
          连接成功后，在 Settings 页面顶部{" "}
          <strong className="text-text-primary">nexu Bot Model</strong> 下拉菜单中选择 Agent
          使用的模型，支持跨供应商随时切换。
        </p>
            ...
            <tbody className="divide-y divide-border">
              {PROVIDERS.map((p) => (
                <tr key={p.name} className="bg-surface-0">
                  ...
                  <td className="px-4 py-2.5 text-text-tertiary font-mono text-[12px]">
                    {p.keyFormat}
                  </td>
                </tr>
              ))}
      ...
      <Callout variant="info">
        添加 BYOK 供应商时，先点击 <strong>Verify Connection</strong> 验证连通性，再保存，确保 nexu
        能正确加载可用模型列表。
      </Callout>

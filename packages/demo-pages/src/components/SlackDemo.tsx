...
/* ------------------------------------------------------------------ */
/*  Demo Scenarios — real nexu use cases                               */
/* ------------------------------------------------------------------ */
    ...
    messages: [
      {
        ...
        time: "10:15 AM",
        attachments: [{ name: "sales-2026-Q1.csv", icon: FileText, color: "text-green-400" }],
      },
      {
        role: "bot",
    ...
    messages: [
      {
        ...
        time: "4:00 PM",
        attachments: [{ name: "changelog-v0.4.md", icon: FileText, color: "text-purple-400" }],
      },
      {
        role: "bot",
    ...
    <div className="flex gap-2 px-5 py-1 hover:bg-[#f8f8f8] group">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex gap-1.5 items-baseline">
          <span className="text-[14px] font-bold text-[#1d1c1d]">{isBot ? "nexu" : "Eli"}</span>
          {isBot && (
          ...
          >
            <div className="px-3 py-2">
              ...
              {msg.result.link && (
                <button
                  type="button"
                  className="inline-block mt-1 text-[13px] text-[#1264a3] hover:underline"
                >
                  {msg.result.link}
                </button>
              )}
...
/* ------------------------------------------------------------------ */
/*  SlackDemo — full Slack client UI with real nexu scenarios          */
/* ------------------------------------------------------------------ */
...
export default function SlackDemo({
  ...
  className,
  height,
}: { scenarioKey: string; className?: string; height?: string }) {
  const scenario = DEMO_SCENARIOS.find((s) => s.key === scenarioKey) ?? DEMO_SCENARIOS[0];
  const activeChannel = scenario.channel.replace("#", "");

  return (
    <div
      className={`overflow-hidden rounded-xl shadow-2xl border border-[#ccc]/30 ${className ?? ""}`}
    >
      {/* Title bar */}
      ...
      <div className={`flex ${height ?? "h-[400px] sm:h-[520px]"}`}>
        {/* Icon rail */}
        <div className="hidden sm:flex w-[34px] shrink-0 bg-[#3f0e40] flex-col items-center py-1.5 gap-2 border-r border-[#5c2d5e]">
          <div className="w-5 h-5 rounded-md bg-white/15 flex items-center justify-center text-[10px] text-white/80">
            🏠
          </div>
          <div className="w-5 h-5 rounded-md bg-white/10 flex items-center justify-center text-[10px] text-white/50">
            💬
          </div>
          <div className="w-5 h-5 rounded-md bg-white/10 flex items-center justify-center text-[10px] text-white/50">
            📁
        ...
        {/* Sidebar */}
        <div className="hidden sm:flex w-[120px] shrink-0 bg-[#3f0e40] flex-col py-2 px-1 border-r border-[#5c2d5e] overflow-hidden">
          <div className="flex items-center gap-1 px-1.5 mb-2">
            <span className="text-[11px] font-bold text-white/90">nexu Team</span>
            <ChevronDown size={10} className="text-white/50" />
          </div>
          <div className="text-[9px] font-medium text-white/40 px-1.5 mb-0.5">Channels</div>
          <div className="space-y-px mb-1.5">
            {SLACK_CHANNELS.map((ch) => {
              ...
              return (
                <div
                  key={ch}
                  className={`px-1.5 py-[2px] rounded text-[10px] truncate ${
                    isActive
          ...
          <div className="text-[9px] font-medium text-white/40 px-1.5 mb-0.5">Direct Messages</div>
          <div className="space-y-px">
            {SLACK_DMS.map((dm) => (
              <div
                key={dm.name}
                className="flex gap-1 items-center px-1.5 py-[2px] text-[10px] text-white/60 truncate"
              >
                <div
                  className={`w-1.5 h-1.5 rounded-full shrink-0 ${dm.online ? "bg-[#2bac76]" : "border border-white/30"}`}
                />
        ...
        <div className="flex flex-col flex-1 min-w-0 bg-white">
          <div className="flex items-center px-5 py-2.5 border-b border-[#e0e0e0]">
            <span className="text-[15px] font-bold text-[#1d1c1d]"># {activeChannel}</span>
          </div>

          <div className="flex-1 overflow-y-auto py-3 space-y-2">
            {scenario.messages.map((msg) => (
              <SlackMsgRow key={`${msg.time}-${msg.content}`} msg={msg} />
            ))}
          </div>
          ...
          <div className="px-5 pb-3">
            <div className="rounded-lg border border-[#ccc] overflow-hidden">
              ...
              <div className="px-3 py-2">
                <span className="text-[13px] text-[#aaa]">Message #{activeChannel}</span>
              </div>

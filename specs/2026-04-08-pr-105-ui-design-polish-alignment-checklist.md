...
- [ ] 审查 `packages/ui-web` 内其余交互性 primitive / pattern，清理 `text-[10px]`、`text-[11px]` 的可点击文本
- [ ] 对 `packages/demo-pages/src/pages/openclaw/*` 做一次 typography audit，优先处理 clickable / selectable / dismissible 元素上的 10px/11px 文本
...
- [ ] 在 Storybook 中新增或更新示例，展示 tabs / pills / filters / compact buttons 的最小字号基线
...
- [ ] 在 `specs/` 中补一份独立规范，固化“Region > Auth tab > Inputs > Save”的 3 层层级
- [ ] 盘点当前仓库是否已有可复用组合：`Card` + segmented control + `FormField` + trailing actions

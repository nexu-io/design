...
- **No shadow changes on button hover**: buttons must not lift or deepen their shadow on `:hover`. Never add `hover:shadow-*` (e.g. `hover:shadow-md`, `hover:shadow-lg`, `hover:shadow-accent/20`) to any button variant or size. Hover feedback must come from `background`, `border`, or `text-color` changes only. Resting shadows (`shadow-xs` / `shadow-sm`) are fine and stay constant through hover. This applies to **all** button variants (`default`, `brand`, `outline`, `soft`, `destructive`, `ghost`, …) and every size including `lg`. Rationale: buttons are already above the page with border + fill affordance — adding a hover-only drop shadow makes them feel like a floating card (double-elevation) and competes with actual Card hover lift. Cards may lift on hover; buttons may not.
